...
GRMHD baselines predict for a natural MAD at the fiducial mass (1e-2 to 1e-1 Hz
...
continuum = f ** -1.2
bump = 8.0 * np.exp(-0.5 * ((np.log10(f) + 1.5) / 0.25) ** 2)   # eruption band centred ~3e-2 Hz
...
ax.axvspan(1e-2, 1e-1, color="#b98a2e", alpha=0.12)
ax.text(3e-2, 2e4, "flux-eruption band\n($10^{2}$--$10^{3}\\,r_{\\rm g}/c$;\n"
                   "$10^{-2}$--$10^{-1}$ Hz at $2\\times10^{4}\\,M_\\odot$)",
