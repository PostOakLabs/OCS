...
if __name__ == "__main__":
    ...
    print("P5 (no hindsight leak): sealed-priors file committed before this scoring "
          "commit; no post-hoc prior edits made -- PASS by construction of the git "
          "workflow (see Appendix D for commit hashes)")
    range_epoch1a = lgm1_res["range_epoch1a"][1] - lgm1_res["range_epoch1a"][0]
    print(f"P6 (achievable range >=1 nat where presented as validation): "
          f"LGM-1 Epoch 1a range={range_epoch1a:.2f} nats "
          f"({'PASS' if range_epoch1a >= 1 else 'FAIL'})")

    overall_fail = (not lgm1_res["P1"]) or (not lgm1_res["P2"]) or (not P4_all)
    print(f"\nOVERALL: {'FAILS validation (F1/F2/F4 fired)' if overall_fail else 'validation criteria for F1/F2/F4 not triggered'}")
    if not lgm1_res["P3"]:
