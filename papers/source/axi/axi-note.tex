...
% session; leave \date at "Draft v0.6 (last revised 2026-08-27)".
%
...
\date{September 2026 \\[4pt] \small Draft v0.7 (last revised 2026-09-03) --- methods note for the AXI campaign;\\ \small companion to the mass-tension paper (Paper H); prepared for omegacentauri.me}
...
\paragraph{H-D: FAIL.} The correlation with per-bin star counts runs $|\rho| = 0.144$ to $0.233$ at $p = 0.147$ to $0.370$, and its radius-controlled partial runs 0.082 to 0.145 against a threshold of 0.30. The surface-density form of the predictor behaves the same way: its raw correlation runs $|\rho| = 0.239$ to $0.323$ and its radius-controlled partial runs 0.273 to 0.283, both under threshold at every reading \citep{AXIStage0Report2026}. The committed stop-or-continue rule stops the campaign only on a passing H-D discriminant, so it did not fire.
...
\paragraph{The A3 non-triviality disclosure, measured rather than argued.} X-G-a's binding condition, that the differential-injection axis be non-trivial for the configuration being scored or the gate reports as vacated, is checked directly. Holding the mock survey identical by the seed pairing of note X-G-a-N5 and varying only the injected differential amplitude moves the \dD{} amplitude coverage from 0.850 to 0.713 in the null member and moves the recovered amplitude from 0.465 to 0.920 \citep{AXIGatesReport2026}. The axis is therefore non-trivial, a coverage number along it is a measurement, and X-G-a reports as failed rather than as vacated. This disclosure travels with the gate whether or not it is quoted elsewhere.
...
In the four rows at $\tD = 0.5$ and the four at $\tD = 1.0$~km\,s$^{-1}$ (eight rows, all three readings each) the maximum $|\dD|$ inside the span is 1.140 to 1.151~km\,s$^{-1}$, outside the region. In the four rows at $\tD = 0.25$~km\,s$^{-1}$ (twelve readings) seven are low, inside the region: the posterior-mean and joint-MAP cells of three configurations (Plummer at 5494 and 5200~pc, $\alpha\beta\gamma$ at 5494~pc), plus the posterior-mean cell alone of the fourth ($\alpha\beta\gamma$ at 5200~pc) — bin-grid maximum 0.335 to 0.412~km\,s$^{-1}$, outer-knot posterior mean 0.345 to 0.427~km\,s$^{-1}$. The remaining five readings at $\tD = 0.25$ are high, outside the region: the profile-leg-best cell in all four configurations (1.140 to 1.144~km\,s$^{-1}$), plus $\alpha\beta\gamma$ at 5200~pc's joint-MAP cell, which splits from its own posterior-mean cell (0.403 low against 1.140 high) rather than following it.

Read per cell, this is the two-mode signature directly: at the halved prior the posterior-mean cell samples the low mode (inside the calibrated region) in all four configurations, the joint-MAP cell joins it in three of the four ($\alpha\beta\gamma$ at 5200~pc instead samples the high mode at its joint MAP, splitting from its own posterior-mean cell), and the profile-leg best cell samples the high mode (outside the region) in every configuration; at the primary and doubled priors every reading samples the high mode. The per-row, per-reading classification is in this note's released record companion (\texttt{calc7/axi\_calc7\_switch.json}); no mechanism statement follows from it.

\begin{table}[htbp]
\centering
\small
\caption{The switch behind the X-G-c failure, at the fiducial-bracket MAP cell, Plummer at 5494~pc. Each row is unimodal in the white systematic $s$; the preferred solution switches between rows. Prior shrinkage cannot produce the move, because the outermost knot carries a posterior standard deviation of 0.0217 to 0.0509~km\,s$^{-1}$ across the full 36-reading battery (0.0217 to 0.0424 for the nine Plummer-5494 readings; 0.0424, 0.0261 and 0.0260 at the three $\tD$ rows in this table's own fiducial-bracket joint-MAP cell). The two right-hand columns are different objects: the bin-grid maximum is evaluated over the forty catalogue bins inside $[5, 300]\as$ (the gate convention; the $300\as$ knot is not a bin), while the outer-knot column is the spline's posterior mean at the $300\as$ knot itself. Both are correctly computed; an earlier version of this table carried only the knot value under a single ``outer knot \dD{}'' heading, where \S\ref{sec:gates-c}'s text separately quotes the bin-grid maximum. The ``mass at $s<0.06$'' column is computed directly from the posterior samples; the \texttt{s\_edge\_mass} field released alongside this note is a separate grid-edge tail-mass check ($10^{-52}$ to $10^{-57}$ in every reading) and is not this divider.}
\label{tab:switch}
\resizebox{\textwidth}{!}{%
...
The mechanism is a degeneracy between \dD{} and the white systematic $s$, and it is measured rather than inferred (Table~\ref{tab:switch}). The outer bins carry a component split that the fit must assign to one term or the other. At $\tD = 0.5$ or $1.0$ the fit assigns the split to a large differential and a small white systematic; at $\tD = 0.25$ it assigns the split instead to a white systematic three times larger, with inflated errors. The switch is bracketed by the two sampled prior widths 0.25 and 0.5~km\,s$^{-1}$; no intermediate $\tD$ was tested, so the transition is bracketed rather than located.
...
The gate was scored two ways, both fixed before the run: against the constant 0.5~km\,s$^{-1}$ scale named for the \dD{} prior, and against the radius-dependent equipartition band it was derived from. It fires if either is exceeded by more than a factor of two anywhere inside the knot span $[5, 300]\as$; the two bins outside the span, where the spline is a linear extrapolation, are excluded from the verdict. Amendment AXI-A3 (Table~\ref{tab:amendments}) records that the equipartition derivation is a statement about the split $\sigma_R - \sigma_T = 2\dD$, not about \dD{} alone; the scorer as coded compares $|\dD|$ to the band directly, a factor of two tighter than the quantity the band was derived to bound. Both conventions are reported.

On the as-coded $|\dD|$-versus-band convention, the maximum $|\dD|$ inside the span runs 0.335 to 1.151~km\,s$^{-1}$ over the 36 readings, at $280.9\as$ in every one of them. Twenty-nine of the 36 readings exceed twice the constant scale, and the same 29 exceed twice the equipartition band, with a maximum ratio to the band of 1.10 to 3.77. On the AXI-A3-consistent $|2\dD|$-versus-band convention, all 36 of the 36 readings exceed twice the band, including the seven low-mode readings of Section~\ref{sec:gates-c} (ratio 2.20 to 2.59), with a maximum ratio of 7.54. \textbf{X-G-d: the clause FIRES on both conventions.} The seven readings that stay inside the band on the as-coded convention are the large-$s$ solutions of Section~\ref{sec:gates-c}, and they exceed the band on the AXI-A3-consistent convention as well, so the two gate outcomes have one mechanism between them under either scoring.

The consequence is applied as written: the component-differential model is declared misspecified beyond this campaign's scope, and the campaign does not escalate to an axisymmetric model on the strength of it. The excess is confined to large radius, at a boundary later than the calibrated-region crossing of Section~\ref{sec:gates-a}: the region boundary itself, $|\dD| = 0.5$~km\,s$^{-1}$, is crossed near $134\as$ interpolated, while the budget-overrun crossing follows at the $201.15\as$ bin ($\approx199\as$ interpolated; 5.36~pc at 5494~pc, 5.07~pc at 5200~pc) on the as-coded convention, or at the $142.43\as$ bin ($\approx137\as$ interpolated) on the AXI-A3-consistent convention. Inside $50\as$, in binned medians over the groups $[5,10)$, $[10,25)$ and $[25,50)\as$, the differential sits at 0.17, 0.09 and 0.04~km\,s$^{-1}$ against a budget of 0.80, 0.76 and 0.72~km\,s$^{-1}$. A four-knot spline that has to reach 1.2~km\,s$^{-1}$ at $300\as$ is absorbing something other than the equipartition asymmetry it was scaled against, at radii where the systematics budget is an order of magnitude above the statistical errors. That is a misspecification statement about M1 at large radius, and under Section~\ref{sec:scope} it is not a statement about the dark component.
...
Four amendments were ruled and ratified during the campaign, all after adoption and all disclosed here as deviations rather than folded in silently (Table~\ref{tab:amendments}). None changes a threshold, a prior or a model. AXI-A1 did change one member's verdict, X-G-f from FAIL to PASS, by a reference repointing rather than by any change to M1 or the data; the other three amendments license what is quotable under a standing FAIL or FIRES verdict (AXI-A2, AXI-A4) or document a scoring convention without rescoring anything (AXI-A3), and change no verdict.

\begin{table}[htbp]
\centering
\small
\caption{Amendments to the plan of record, all ratified by the project owner. AXI-A1 through AXI-A3 are dated 2026-08-15; AXI-A4 is dated 2026-09-03, ruled during this note's referee-round repair rather than during the campaign itself. Each was ruled by the author of the gate whose scope was in question, on the precedent the plan's amendment protocol names.}
...
AXI-A3 & documentation clarification & records that the equipartition derivation is a statement about the split $\sigma_R - \sigma_T$, which under M1's convention is $2\dD$, so the \dD{} knot prior is a factor of two looser than the derivation's literal edge & the campaign's own fit surfaced the ambiguity. The looser reading is the conservative direction, with less shrinkage and wider intervals, and every gate was scored on the prior as run, so no rerun follows \\[3pt]
AXI-A4 & quotability override, dated 2026-09-03 & states explicitly, against Section~4.3's ``all gates must pass'' rule, what this note may quote while X-G-a stands FAIL and X-G-c and X-G-d stand FAIL/FIRES: the \dD{} amplitude inside the calibrated region (X-G-a's own regional consequence clause, AXI-A2), and the criterion-4 outcome with the X-G-c mechanism named (X-G-c's and X-G-d's own consequence clauses). No dark-component number, no mechanism identification and no compact-versus-extended statement is licensed by this override & the note as drafted offered these two statements as quotable without a rule change from Section~4.3 authorizing the offer explicitly; each was already licensed by its own gate's consequence clause, so this amendment states that licence rather than creating one \\
\bottomrule
\end{tabular}
\end{table}

A fifth process note belongs with them. The standing note added with AXI-A1 requires any future gate or amendment in this campaign that names a numerical reference to name a post-reconciliation artifact, or to state explicitly which kernel that artifact was produced under.
...
\paragraph{Residual systematics.} Stage 0 could test H-D only against per-bin star counts and their surface-density form, the two proxies the frozen radial table carries, so the orientation-dependent reading of the hypothesis is untested here rather than excluded. A consumer wanting to bound it should run the split against per-epoch and detector-geometry predictors in the parent catalogue \citep{Haberle2024oMEGACatII,Haberle2025oMEGACatVI}; nothing in this record licenses treating H-D as closed.

\paragraph{Cross-calibration with Paper H.} This campaign's mocks carry the profile leg and their coverage of the injected dark component sits below the required band (Section~\ref{sec:gates-a}), while Paper H's calibration passes only after the profile leg is dropped \citep{HCalReport2026}. The two results score different configurations, and each holds for the configuration it was run on. The compression measured here reproduces the shared-model compression Paper H measured before any differential block existed, which is the property Paper H removed by dropping the leg, so the two records agree once the configuration is named.
...
\paragraph{Compute.} The twelve fits took 1.2 minutes of wall clock on CPU, dominated by four model-table builds at 5 to 9 seconds each, because the base tables are shared across the three prior rows and M1's marginalisation is closed-form; the figure pass added a further 1.5 minutes. The gate battery is where the cost sits: the main $n = 400$ battery (nine members, 3600 rows) ran 51.2 minutes, and the pre-committed extension ran a further 47.7 minutes at eight workers, 1200 new (injection, $k$) groups producing 3600 new rows (three members per group), at 1.38 to 2.47~s per row.
