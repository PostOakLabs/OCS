...
% BUILDER DRAFT (WU OCS-G-DRAFT-1). Version line owned by the editorial
% session; leave \date at "Draft v0.0 (builder draft, unverified)".
%
...
\date{August 2026 \\[4pt] \small Draft v0.1 (last revised 2026-08-14) --- eighth paper of the set (Paper G); companion to the Macro\\ \small Transcension Hypothesis (Paper A), the inward-migration review (Paper B), the Omega Centauri campaign\\ \small (Paper C), the migration economics (Paper D), the engineered-IMBH systems paper (Paper E), the joint\\ \small accretion bound (Paper F), and the mass-tension paper (Paper H); prepared for omegacentauri.me}
...
Two things follow from doing the cross-match systematically. The first is a set of associations with stated false-association rates, which the partial treatments do not carry. The second is what is left over: a residual population of unidentified sources that survives every known counterpart class and the expected extragalactic background. The residual is the object of interest, and it is worth stating at the outset what kind of object it is. It is a target list. A count of unidentified sources in excess of a background model is a statement about the completeness of identification work in a crowded, unevenly exposed field, and it constrains nothing on its own.

The referee-facing risks in a paper of this kind are well known and each is addressed mechanically rather than rhetorically. Positional cross-matching in a crowded core without a false-association calibration is uninterpretable, so every probe carries a chance-coincidence probability from three independent estimators (Section~\ref{sec:method-chance}). Colour-box classification without membership priors is arithmetic without content, so membership enters as a probability under stated external normalisations and never as a binary label (Section~\ref{sec:method-membership}). Extragalactic contamination dominates a field this deep, so the background enters through a published log~$N$--log~$S$ relation with its parameter uncertainties propagated as a bracket (Section~\ref{sec:method-agn}). Fluxes near the detection limit are biased high, which we state as a direction of error rather than correct. And the whole pipeline is validated on a control cluster before it is allowed to produce a number for \ocen{} (Section~\ref{sec:control}).

Section~\ref{sec:data} fixes the catalogues and their provenance. Section~\ref{sec:method} gives the cross-match, the three chance estimators, the frame registration, the membership treatment, and the background accounting. Section~\ref{sec:control} reports the control validation, which is a result and not a preamble. Section~\ref{sec:results} gives the \ocen{} cross-match, the discrepant association, and the residual census. Section~\ref{sec:tension} carries the population-model disagreement. Section~\ref{sec:discussion} states what would identify the residual. Appendix~\ref{app:caveats} collects the caveats as a checklist.
...
The limitation is structural and we state it rather than working around it. The anchors are published associations, so re-matching those same pairs after registration is not an independent test of them. The registration earns its place on the probes that are not anchors, where it shifts the search position by a measured amount, and on the resulting change in the tier-1 match count.
...
The escape-velocity number exceeds the residual of 12.1 by more than an order of magnitude, and exceeds the full unidentified count of 159 once the background is not subtracted. That bounds something. It bounds the extrapolation, or the assumed detection fraction, or the background subtraction, and this analysis does not decide which. Three reasons keep each candidate live.
...
We carry the disagreement as a disagreement. Adjudicating it requires the identification work this census is meant to enable, not a preference asserted here.
