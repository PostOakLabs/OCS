...
% Paper D of the set. Target: omegacentauri.me -> arXiv.
...
\date{September 2026 \\[4pt] \small Draft v2.1 (last revised 2026-09-03) --- fourth paper of the set; companion to \emph{The Macro Transcension Hypothesis} (Paper A), the inward-migration review (Paper B),\\ \small the Omega Centauri campaign (Paper C), the engineered-IMBH systems paper (Paper E), the joint accretion bound (Paper F),\\ \small the residual X-ray census (Paper G), and the mass-tension paper (Paper H);\\ \small prepared for omegacentauri.me}
...
The gap is specific. \citet{Bostrom2003} priced delay: every century of postponed expansion forgoes an irrecoverable harvest of free energy. \citet{Bennett2019} priced dormancy: free energy not collected promptly is lost, so the aestivation strategy of \citet{Sandberg2016} forfeits effectively all of what it hoped to save. Relocation has not been priced. A lineage that migrates abandons its accumulated local infrastructure, spends $10^{4}$--$10^{6}$ years in transit, accepts a risk of total loss en route, and only then begins to collect a payoff that is, by the physics of Paper A, orders of magnitude larger per unit of everything. Whether that trade clears depends on quantities the inward-family literature has so far left unmodelled: discount rates, hazard rates, transit risk, and the option of sending a seed instead of moving.

This matters beyond bookkeeping. If migration clears the trade only for implausibly patient agents, the inward family loses its force as a Fermi solution regardless of the destination physics. If it clears easily, the family's incomplete-compliance problem (Paper B, problem P4) sharpens into a quantitative question: what fraction of lineages must migrate for the observed silence to follow? Both outcomes constrain the family.

\subsection{Epistemic register}
\label{sec:claims}
...
We do not claim the parameter priors are measurable; the contribution is the structure of the crossover the analysis exposes, with point estimates serving only as orientation. We flag model dependence throughout (Section~\ref{sec:limits}).
...
\label{eq:utility}
\end{equation}
where $C_{\pi}(t)$ is the computation rate delivered at time $t$ (bit-erasures per second, the binding currency by the Landauer accounting of Paper A, Section 4.2), $t$ is measured in years throughout ($\rho$, $\lambda$, and $\tau$ below are all yr and yr$^{-1}$; the erasures-per-second rates of Section~\ref{sec:kernels} are annualized by the implicit factor of $3.156\times10^{7}$~s\,yr$^{-1}$ before entering $U$, which cancels in every dimensionless ratio used below and is stated once here rather than at each occurrence), and $\rho \geq 0$ an exponential discount rate. We use exponential discounting for tractability and revisit hyperbolic alternatives \citep{Laibson1997} in Section~\ref{sec:limits}; the patient limit $\rho \to 0$ recovers the \citet{Ramsey1928} zero-discount benchmark, an argument \citet{Bostrom2003} makes for agents with unbounded horizons.

For readers who have not worked with exponential discounting: the parameter $\rho$ measures impatience. A payoff delivered $\tau$ years from now is worth $e^{-\rho\tau}$ times the same payoff today. At the fiducial $\tau = 10^{5}$~yr, even a modest $\rho = 10^{-4}$~yr$^{-1}$ discounts the destination payoff by $e^{-10} \approx 5\times10^{-5}$ --- a factor of 20{,}000. The migration question is whether the destination advantage $G$ overcomes this factor, and the answer depends logarithmically on $G$ but linearly on $\rho$ and $\tau$.

Three strategies span the space of single-destination policies:
...
\item[H (seed and stay).] Dispatch a self-replicating seed payload \citep{Freitas1980,Armstrong2013} of negligible mass while executing S at home; the seed bootstraps the destination in parallel. The lineage pays a small fraction $f$ of local output for the launch infrastructure and collects both payoffs, with the deferred term weighted by a goal-drift valuation $p_{d}$ introduced in Section~\ref{sec:kernels}; the seed's transit hazards (radiation damage, replication error) also differ in kind from those facing a deliberating lineage in transit.
\end{description}

Strategy H is simply the architecture Paper A already assumes (gram-to-kilogram scouts, then seed factories), read as an economic option rather than a mission profile. What has not been stated before is its dominance structure (Section~\ref{sec:hybrid}). Multi-destination seeding, one lineage dispatching $k > 1$ seeds to distinct clusters, generalizes H directly: the per-lineage cost scales to $kf$ and the hybrid condition of Section~\ref{sec:hybrid} applies seed by seed. This paper does not model that generalization separately.

\subsection{Parameters}
\label{sec:parameters}

Table~\ref{tab:params} collects the parameters and fiducial values. Transit time uses Paper A's beamed-sail speeds ($0.1$--$0.2\,c$) and the distance to $\omega$~Cen ($5.49$~kpc; \citealt{Haberle2025oMEGACatVI}) as the fiducial destination, giving $\tau_{\rm transit} \approx 0.9$--$1.8\times10^{5}$~yr; we fold construction ($\lesssim 10^{3}$~yr to first ISCO-swarm operation; Paper A, staged-construction appendix) into a single $\tau = 10^{5}$~yr fiducial. The bare cruise time at the tabulated $v = 0.15c$ is $d/v = 1.19\times10^{5}$~yr, so the round fiducial corresponds to an effective $\approx 0.18c$; Table~\ref{tab:params} states the bare number so the rows can be checked independently. We keep $\tau_{\rm fid} = 10^{5}$~yr as the printed fiducial rather than the bare $1.19\times10^{5}$~yr, so the threshold understates the true transit time by at most $19$~per~cent before construction time is added back in; the residence-survival debit is insensitive to this choice, differing by less than $2\times10^{-6}$ between the two readings of $\tau$. Beamed-sail launch provides no deceleration at an unprepared destination, and braking by magsail \citep{ZubrinAndrews1991,PerakisHein2016} or by photon-assisted maneuvers \citep{HellerHippke2017} can multiply the cruise time by a factor of a few; because the threshold of Section~\ref{sec:crossover} scales linearly in $1/\tau$, we propagate this allowance explicitly there (Table~\ref{tab:tausens}) rather than absorbing it into the range. The hazard rate $\lambda$ prices the goal-stability problem (Paper B, problem P2): the probability per year that the lineage ceases to pursue the plan, through value drift, fragmentation, or extinction. We do not claim to know $\lambda$; it enters the thresholds additively with $\rho$, which is the analytically convenient way to carry an unknown.

\begin{table}[tbp]
\centering
\caption{Decision parameters and fiducial values. Two entries carry cross-paper dependencies: the residence-survival figure imports Paper E's flyby accounting, whose operative channel is now the bound-cusp diffusion floor rather than the (non-operative) adiabatically corrected unbound-field curve, and is bimodal rather than a single number (Section~\ref{sec:crossover}), and the $G = 10^{6}$ variant imports Paper E's ambient-supply estimate, which falls to $G \approx 10^{3}$ under Paper E's ADIOS-suppressed ceiling of $2.6\times10^{3}\,L_\odot$ \citep{Swanson2026Engineered} (Section~\ref{sec:kernels}).}
\label{tab:params}
\small
\begin{tabular}{l p{4.4cm} p{3.2cm} p{4.6cm}}
\toprule
...
$p_{s}$ & transit survival probability (deliberating lineage) & $0.5$ & \citet{Hoang2017} \\
$p_{s}^{\rm seed}$ & transit survival probability (seed payload) & $0.5$ adopted pending dedicated modelling & seed hazards (radiation, replication error) differ in kind from lineage transit hazards (Section~\ref{sec:kernels}); no separate estimate exists, so $p_s^{\rm seed} = p_s$ is a placeholder, not a derived equality \\
$G$ & destination rate multiplier & $10^{9}$ & power ratio; $10^{6}$ without fuel imports, $10^{3}$ under ADIOS suppression (Section~\ref{sec:kernels}) \\
$\rho$ & discount rate & free & \citep{Ramsey1928,Bostrom2003} \\
$\lambda$ & goal-stability hazard rate & free & Paper B, problem P2 \\
$f$ & seed cost, fraction of S output & $10^{-6}$ adopted (physical bound $\ll10^{-3}$) & gram--kg payloads \citep{Lubin2016,Freitas1980} \\
$p_{d}$ & goal-drift valuation of seed output & free, $\lesssim 1$ by engineering & Section~\ref{sec:kernels} \\
$p_{d}^{\mathrm M}$ & goal-drift valuation of a deliberating migrant's own output & free & rescues $U_{\mathrm H} \geq U_{\mathrm M}$ only if $p_d \geq p_d^{\mathrm M}$ (Section~\ref{sec:hybrid}) \\
\bottomrule
\end{tabular}
\end{table}

See Paper F App.~A for the shared-parameter table across the set; this paper's distance fiducial $d = 5.49$~kpc matches the set consensus there.
...
At the destination, the sustainable power is the Eddington luminosity of the host, $L_{\rm Edd} \simeq 2.5\times10^{35}$~W at $2\times10^{4}\,\msun$ (Paper A, Eq.~2), a factor $\sim$$10^{9}$ ($6.6\times10^{8}$, rounded) over the solar Dyson ceiling. The erasure cost against a horizon sink falls by a further factor $\sim$$10^{12}$ (Paper A, Section 4.2), but that factor is a floor-to-floor comparison of theoretical erasure minima, not a realized rate; Paper A (\S4.2) is explicit that the realized gain, power delivered per unit time at the destination relative to the source, is $10^{6}$--$10^{9}$, and states that the $\sim$$10^{12}$ figure should not be quoted ``wherever the realized gain is meant.'' $G \equiv C_{\mathrm M}/C_{\mathrm S}$ is exactly that realized gain, so the erasure-floor factor does not belong in it: the admissible multiplier on Eq.~(\ref{eq:cs}) is the power ratio alone, times the realized (not floor-to-floor) erasure gain of $10^{6}$--$10^{9}$, giving a Paper-A-compliant realized band of $6.6\times10^{14}$--$6.6\times10^{17}$ (rounded, $\sim$$10^{15}$--$10^{18}$). We adopt
\begin{equation}
G \;\equiv\; \frac{C_{\mathrm M}}{C_{\mathrm S}} \;=\; 10^{9}
\label{eq:g}
\end{equation}
as the fiducial, crediting the migration payoff with the power ratio only (rounded up from the recomputed $6.6\times10^{8}$, a $+2.1$\% anti-conservative rounding on $\rho^{*}$ that is dwarfed by the order-of-magnitude uncertainties carried elsewhere in $G$). The Eddington figure is a capacity, not a fuel supply: the infrastructure companion \citep[hereafter Paper E]{Swanson2026Engineered} budgets the ambient Bondi-fed accretion available in a gas-poor cluster at $\sim$$10^{6}\,L_\odot$, so the power-alone multiplier is $G \sim 10^{6}$ unless the civilization imports fuel, which raises $G$ at the cost of visibility (enhanced accretion luminosity is one of Paper E's residues). The recurring logistics of that import programme, chiefly angular-momentum removal from infalling mass (priced in Paper A), are not carried in the kernel; they are absorbed into the conservatism of the power-only $G$, while the far smaller seed cost $f$ is carried explicitly because it gates the hybrid boundary of Section~\ref{sec:hybrid}. At $G = 10^{6}$ the threshold of Section~\ref{sec:crossover} becomes $\rho^{*} \simeq \ln(5\times10^{5})/10^{5}\ \mathrm{yr} \approx 1.3\times10^{-4}$~yr$^{-1}$, against $2.0\times10^{-4}$ at $G = 10^{9}$: three orders of magnitude in the multiplier move the threshold by a factor of 1.5, the logarithmic robustness on which the analysis leans. The $G = 10^{6}$ figure imports Paper E's ambient-supply estimate and moves with it: under the ADIOS outflow-suppression standard for hot low-Eddington flows \citep{Swanson2026Engineered}, the delivered rate falls to a ceiling of $2.6\times10^{3}\,L_\odot$, giving $G \approx 10^{3}$ and moving the threshold to $\rho^{*} \approx 0.6\times10^{-4}$~yr$^{-1}$, the same order. Two further reasons for the understatement. First, horizon-sink disposal shifts the binding constraint to waste-heat transport (Paper A, Section 4.2), whose engineering efficiency is unmodelled; using the power ratio alone keeps the kernel on demonstrated physics. Second, every conclusion below strengthens monotonically in $G$, so a conservative $G$ makes the thresholds conservative. Readers preferring the full realized-erasure-gain stack may substitute $G = 6.6\times10^{14}$--$6.6\times10^{17}$ (the Paper-A-compliant ceiling); the thresholds move by the logarithm, i.e.\ by a factor of $\leq 2.0$ (Section~\ref{sec:crossover}). The floor-to-floor $\sim10^{21}$ figure is a thermodynamic ceiling on erasure cost alone, not a realized payoff, and is not substituted here. One accounting caveat: if computation at either site is performed reversibly \citep{Bennett1973}, erasures are not proportional to useful computation and the two sites' conversion factors could in principle differ. Because $G$ is a ratio of erasure budgets, architecture-dependent reversibility factors cancel to first order when both sites use the same logic family, which is the natural assumption for a single lineage.
...
Equations~(\ref{eq:us})--(\ref{eq:um}) also let both payoff streams run forever, and the physical horizons differ. Strategy S is bounded by the host star's remaining main-sequence life, $T_{\mathrm S} \approx 5\times10^{9}$~yr for a solar analogue. Star lifting extends this horizon substantially only for fully convective low-mass hosts, to $T_{\mathrm S} \sim 10^{11}$--$10^{12}$~yr; a solar analogue, the fiducial host here, gains only $\sim3\times10^{9}$~yr from the same mechanism \citep{ScogginsKipping2023}. Strategy M at the Eddington rate consumes roughly one solar mass per $2{,}300$~yr at $\eta = 0.1$ and exhausts an accessible reservoir of $10^{5}$--$10^{6}\,\msun$ in $T_{\mathrm M} \approx 2.3\times10^{8}$--$2.3\times10^{9}$~yr; this reservoir range is adopted for round numbers rather than derived, and sits $4$--$40\times$ below Paper A's working cluster-mass reservoir, a conservative (shorter-$T_{\mathrm M}$) choice whose only downstream effect is the finite-horizon cap below. With finite horizons the ratio becomes
\begin{equation}
...
\end{equation}
Three consequences follow. At and near the threshold of Section~\ref{sec:crossover}, $(\rho+\lambda)T_{\mathrm M} \gtrsim 4\times10^{4}$, so both brackets equal 1 to exponential accuracy and $\rho^{*}$ is unchanged; the cancellation argument holds for all $\rho + \lambda \gtrsim$ a few $\times10^{-8}$~yr$^{-1}$ (a few $\times10^{-9}$~yr$^{-1}$ only at the long-$T_{\mathrm M}$ end of the range: at the short end, $T_{\mathrm M} \approx 2.3\times10^{8}$~yr, $\rho+\lambda = 3\times10^{-9}$~yr$^{-1}$ gives $(\rho+\lambda)T_{\mathrm M} = 0.69$ and the bracket $1-e^{-0.69} = 0.50$, not 1). In the patient limit $\rho + \lambda \to 0$ the bracket's value depends on which $T_{\mathrm S}$ horizon is operative. Against the unlifted horizon ($T_{\mathrm S} \approx 5\times10^{9}$~yr, the solar-analogue fiducial), the bracket is $T_{\mathrm M}/T_{\mathrm S} \in [0.046, 0.46]$ (from $T_{\mathrm M} \approx 2.3\times10^{8}$--$2.3\times10^{9}$~yr, l'Hôpital's rule on Eq.~\ref{eq:finitehorizon} at $\rho+\lambda \to 0$), capping the migration advantage at $\sim$$p_{s} G\, T_{\mathrm M}/T_{\mathrm S} \approx 2.3\times10^{7}$--$2.3\times10^{8}$. Against the star-lifted horizon for a fully convective low-mass host ($T_{\mathrm S} \sim 10^{11}$--$10^{12}$~yr, an illustrative alternative to the fiducial rather than a bound the solar-analogue case reaches; Section~\ref{sec:kernels}), the bracket shrinks and the cap instead falls to $\sim1.15\times10^{5}$--$1.15\times10^{7}$, rather than letting the advantage grow without bound in either branch. And the finite-horizon kernel is what makes the $\rho + \lambda \to 0$ Ramsey benchmark well-defined at all, since the infinite-horizon utilities (\ref{eq:us})--(\ref{eq:um}) individually diverge there. One coupling worth stating: $G$ and $T_{\mathrm M}$ anticorrelate at fixed reservoir, because the multiplier is set by the burn rate; the Bondi-fed $G = 10^{6}$ variant consumes $\sim$$10^{3}$ times more slowly, giving $T_{\mathrm M} \sim 10^{12}$~yr $> T_{\mathrm S}$, so the patient-limit cap does not bind that branch.

A related asymmetry concerns the seed. Equation~(\ref{eq:uh}) treats seed arrival as all-or-nothing through $p_{s}$, but a seed can arrive functional with drifted goals, yielding computation the home lineage only partially values, and it carries its own transit-survival probability $p_{s}^{\rm seed}$ rather than the deliberating lineage's $p_{s}$ (Table~\ref{tab:params}). A continuous treatment multiplies the hybrid's deferred term by a drift-weighted valuation $p_{d} \in [0,1]$, the expected fraction of the destination payoff the deciding lineage credits after goal drift in the seed line, and substitutes $p_s^{\rm seed}$ for $p_s$ in the deferred exponential:
\begin{equation}
U_{\mathrm H} \;=\; (1-f)\,U_{\mathrm S} \;+\; p_{d}\,p_{s}^{\rm seed}\, e^{-(\rho+\lambda)\tau}\, \frac{G\,C_{\mathrm S}}{\rho+\lambda}.
\label{eq:uhcontinuous}
\end{equation}
Because a seed does not deliberate en route, its drift channels (radiation damage to stored values, replication error during bootstrap) differ from the institutional-drift channels of a deliberating lineage in transit, and plausibly $p_{d}$ is close to 1 by engineering; but nothing below requires that, since $p_{d}$ enters the hybrid condition (Eq.~\ref{eq:hybrid}) inside the same logarithm as $G p_{s}$. A deliberating lineage that migrates is exposed to the same institutional-drift channel over its own exploitation horizon; we denote the corresponding valuation $p_{d}^{\mathrm M}$, so that $U_{\mathrm M}^{\rm drift} = p_{d}^{\mathrm M}\,U_{\mathrm M}$ (Table~\ref{tab:params}). Comparing Eq.~(\ref{eq:uhcontinuous}) with $U_{\mathrm M}^{\rm drift}$ gives the general H-versus-M breakeven,
\begin{equation}
(1-f)\,U_{\mathrm S} \;>\; \big(p_{d}^{\mathrm M}\,p_{s} \,-\, p_{d}\,p_{s}^{\rm seed}\big)\,\frac{G\,C_{\mathrm S}}{\rho+\lambda}\,e^{-(\rho+\lambda)\tau},
\label{eq:hvsmgeneral}
\end{equation}
which reduces to the one-line comparison of Section~\ref{sec:hybrid} when $p_s^{\rm seed} = p_s$ and collapses further to a sufficient condition $p_d \geq p_d^{\mathrm M}$ when the right-hand side is bounded above by zero. Equation~(\ref{eq:uh}) treats the two payoff streams as additive, which is exact if the lineage's utility is linear in computation and the seed's draw on local resources is the fraction $f$; Section~\ref{sec:limits} discusses when additivity fails (identity-centred utilities that do not value a forked descendant's computation).
...
\begin{equation}
\boxed{\;\rho + \lambda \;<\; \frac{\ln\!\big(G\,p_{s}\big)}{\tau}\;\equiv\;\rho^{*}, \quad \text{valid for } G\,p_{s} > 1.}
\label{eq:threshold}
\end{equation}
At $G p_s \leq 1$ the logarithm is non-positive and Eq.~(\ref{eq:threshold}) has no solution with $\rho+\lambda \geq 0$: migration never clears regardless of patience. The threshold depends on the enormous multiplier only through its logarithm: the structural reason the result is robust to the order-of-magnitude uncertainties in $G$. At the fiducials ($G = 10^{9}$, $p_{s} = 0.5$, $\tau = 10^{5}$~yr),
\begin{equation}
\rho^{*} \;=\; \frac{\ln(5\times10^{8})}{10^{5}\ \mathrm{yr}} \;\simeq\; 2.0\times10^{-4}\ \mathrm{yr^{-1}},
\end{equation}
corresponding to a combined discount-plus-hazard half-life $t_{1/2} = \ln 2/\rho^{*} \approx 3{,}500$~yr; this is a planning half-life in the usual sense only under $\lambda = 0$, since the threshold binds the sum $\rho + \lambda$, not the discount rate alone. Any lineage whose effective concern horizon exceeds a few millennia (a modest demand against the $10^{8}$-yr coherence the family's architectures assume elsewhere) prefers migration to densification. Under the Paper-A-compliant realized-gain ceiling ($G = 6.6\times10^{17}$) the threshold rises to $\rho^{*} \approx 4.0\times10^{-4}$~yr$^{-1}$ ($t_{1/2} \approx 1{,}700$~yr); under a pessimistic $p_{s} = 0.01$ it falls only to $1.6\times10^{-4}$~yr$^{-1}$, and even $p_{s} = 10^{-4}$ moves it only to $1.15\times10^{-4}$~yr$^{-1}$, a factor of 1.7; transit risk is nearly irrelevant inside the logarithm and the operative lever is the transit time $\tau$. Both threshold and half-life additionally require $G p_{s} > 1$: at $G p_s \leq 1$ the logarithm is non-positive and no discount-plus-hazard rate clears the trade (Section~\ref{sec:crossover} below returns to this floor).

Because $\rho^{*} \propto 1/\tau$ linearly, $\tau$ is the one parameter where a factor of a few moves the answer, and the deceleration allowance of Section~\ref{sec:parameters} acts on $\tau$ directly. Table~\ref{tab:tausens} propagates it: if braking multiplies the cruise time by 2--3, the threshold falls to $0.7$--$1.0\times10^{-4}$~yr$^{-1}$ and the headline half-life lengthens to $\sim$$7$--$10$~kyr. The qualitative conclusion (patience horizons of millennia, against the $10^{8}$-yr coherence assumed elsewhere in the family) is unchanged; the quoted $3{,}500$-yr figure is the no-braking end of the band. These single-factor sensitivities are not independent: the joint pessimistic corner, ADIOS-suppressed $G = 10^{3}$ combined with braked $\tau = 3\times10^{5}$~yr, gives $\rho^{*} = \ln(500)/(3\times10^{5}) \approx 2.07\times10^{-5}$~yr$^{-1}$ ($t_{1/2} \approx 33{,}500$~yr), a factor of $\sim$10 below the headline threshold and $\sim$3 below either single-factor pessimization alone. Worse, at $G = 10^{3}$ the tabulated low end $p_{s} = 10^{-4}$ (Section~\ref{sec:crossover} above) gives $G p_{s} = 0.1 < 1$: Eq.~(\ref{eq:threshold})'s validity condition fails, the logarithm is negative, and no discount-plus-hazard rate makes migration pay in that corner.
...
The $p_{s} = 0.5$ fiducial is anchored, loosely, in the only quantitative literature on transit attrition: \citet{Hoang2017} model gas and dust damage to relativistic spacecraft and find survivable but non-negligible erosion at $0.2c$ over the parsec-scale column to $\alpha$~Centauri. Over kiloparsec columns the damage integral is $\sim$$4\times10^{3}$ times larger and mitigation (shielding, redundancy, en-route repair) is unmodelled, so 0.5 should be read as an order-unity placeholder rather than a conservative bound; the logarithmic dependence just exhibited is what makes this imprecision tolerable. Note that $p_{s}$ prices transit alone; residence survival does not enter $\rho^{*}$ (Eq.~\ref{eq:threshold}) and is reported here only as a check that the destination itself holds up once occupied. Paper E's flyby accounting with mass-segregated remnants carries two channels for the operating envelope. The unbound stellar field is adiabatically decoupled from every orbit an installation could occupy (the adiabatic parameter runs from $2.2$ at the stripping radius to $4\times10^{3}$ at the ISCO); the correction that would apply to it, an envelope minimum of $3\times10^{8}$~yr reaching the cluster-age cap in the interior, is retained by Paper E only as the measure of that decoupling and does not operate physically. The channel that does operate is bound-cusp diffusion: a bound $31\,\msun$ remnant is present at the envelope edge in $53$ per cent of Monte Carlo histories, giving, per Paper E's appendix, a diffusion floor with median $4.3\times10^{7}$~yr (16th percentile $2.2\times10^{7}$~yr) in that branch, while a separate $27$ per cent of histories, per that same appendix, reach the $12$-Gyr cluster-age cap with no bound remnant drawn (Paper E's appendix figure; the main text's $47$ per cent is superseded by it for this statistic). Residence survival to the $10^{8}$-yr horizon is therefore bimodal rather than a clean $\approx0.8$: in the majority branch the diffusion floor sits below that horizon by a factor of a few, so survival there is not assured by the flyby channel alone and would need the episodic-hazard and engineered-response accounting below rather than a single multiplicative $p_{s}$; the remaining, cap-limited branch is survival for draws with an absent or distant bound remnant. Because $p_{s}$ prices transit alone by definition (Eq.~\ref{eq:threshold}), this figure cannot enter $\rho^{*}$ by construction, not because residence turns out not to matter: the insensitivity is a scoping fact about what the threshold measures, not a finding about residence durability. Section~\ref{sec:crossover}'s threshold and half-life numbers are unaffected by the correction; what changes is only the confidence with which the residence phase, once reached, is described as durable. This paper does not attempt a residence-survival calculation folding the bimodal figures into a multiplicative correction on $p_s$ or on the deferred payoff; that calculation is Paper E's appendix-dependent territory (episodic TDE hazard, engineered response) and is not carried here. The dominant recurrent hazard, tidal-disruption-event loading with recurrence $\gtrsim 10^{7}$~yr, is survivable by engineered response and enters the deferred payoff only as a small amortization correction on that horizon \citep{Swanson2026Engineered}. The logarithmic entry of $p_{s}$ just exhibited is what keeps the threshold stable against transit-side revisions; it says nothing about the bimodal residence question just raised.
...
\end{loglogaxis}
\end{tikzpicture}
\caption{The patience threshold $\rho^{*} = \ln(G p_{s})/\tau$ (Eq.~\ref{eq:threshold}) versus destination distance, for the beamed-sail speed range and fiducial $G p_{s} = 5\times10^{8}$. A lineage migrates when its combined discount-plus-hazard rate falls below the curve. Named points assume the marker's cluster is the nearest adequate destination for the deciding lineage; distances from \citet{BaumgardtVasiliev2021} (47~Tuc, M54) and \citet{Haberle2025oMEGACatVI} ($\omega$~Cen, matching the Table~\ref{tab:params} fiducial). The logarithmic dependence on $G p_{s}$ keeps the curves' movement modest: one decade in $G p_{s}$ moves $\rho^{*}$ by $\sim$12~per~cent, and the full adopted $G$ bracket ($10^{3}$--$6.6\times10^{17}$, ADIOS floor to Paper-A-compliant realized ceiling) moves it by a factor of $\sim$6.5. Both curves use the bare cruise time $\tau = d/v$; deceleration multiplying $\tau$ by 2--3 lowers them by the same factor (Table~\ref{tab:tausens}).}
\label{fig:crossover}
\end{figure}

\subsection{Meaning of the threshold}
...
Comparing Eq.~(\ref{eq:uh}) with Eqs.~(\ref{eq:us}) and (\ref{eq:um}): under additive utility and a full-fidelity seed ($p_d \geq p_d^{\mathrm M}$, $p_s^{\rm seed} = p_s$; Section~\ref{sec:kernels}), $U_{\mathrm H} > U_{\mathrm M}$ in one line (the stay component is pure addition), and $U_{\mathrm H} > U_{\mathrm S}$ whenever
\begin{equation}
f \;<\; G\, p_{s}\, e^{-(\rho+\lambda)\tau}, \quad \text{under additive utility and } p_d \geq p_d^{\mathrm M}.
\label{eq:hybrid}
\end{equation}
The right-hand side at fiducials is $\sim$$5\times10^{8}\,e^{-20\,(\rho+\lambda)/\rho^{*}}$, and the exponential is unforgiving: at the migration threshold itself the tolerable seed cost is $5\times10^{8}e^{-20} \sim 1$, but at $\rho + \lambda = 2\rho^{*}$ it has fallen to $\sim$$2\times10^{-9}$, and at $10\rho^{*}$ to $\sim$$10^{-78}$. The physical seed cost, gram-to-kilogram payloads plus a launch array \citep{Lubin2016,Freitas1980}, is tiny against a Dyson-scale economy: \citet{Armstrong2013} estimate that seeding every galaxy in the reachable universe costs hours of one star's output, which supports $f \lesssim 10^{-6}$ a fortiori for a single Galactic destination. Setting $f = 10^{-6}$ in Eq.~(\ref{eq:hybrid}) gives breakeven at $(\rho+\lambda)\tau = \ln(5\times10^{14}) \approx 34$, i.e.\ $\rho + \lambda \approx 1.7\,\rho^{*}$. The hybrid therefore dominates both pure strategies in a band extending modestly beyond the migration threshold, roughly $1.7\times$ the threshold at $f = 10^{-6}$ (and only logarithmically further for still-cheaper seeds), because the exponential discount on the deferred payoff crushes the seed's expected value within a factor of a few of $\rho^{*}$. Outside that band the utility calculus reverts to pure S; inside it, and everywhere migration itself clears, H is strictly best barring non-additive utilities (Section~\ref{sec:limits}). Figure~\ref{fig:phasespace} maps the three regimes in the $(G,\ \rho+\lambda)$ plane.

\begin{figure}[tbp]
\centering
\includegraphics[width=0.8\textwidth]{figs/fD_phasespace.pdf}
\caption{The phase space of the migration decision in the $(G,\ \rho+\lambda)$ plane, at $p_{s} = 0.5$ and $\tau = 10^{5}$~yr (Table~\ref{tab:params}). Green: migration clears, $\rho+\lambda < \rho^{*} = \ln(G p_{s})/\tau$ (Eq.~\ref{eq:threshold}); within it the hybrid weakly dominates pure M under the seed-fidelity assumption. Orange: the hybrid band, where the hybrid beats S while pure M does not, extending to $\rho+\lambda = \ln(G p_{s}/f)/\tau = 1.7\,\rho^{*}$ at the fiducial seed cost $f = 10^{-6}$ (Eq.~\ref{eq:hybrid}). Blue: S dominates; neither migration nor seeding pays. The star marks the fiducial point ($G = 10^{9}$, $\rho+\lambda = 2\times10^{-4}$~yr$^{-1}$), which sits on the migration threshold. The plotted $G$ axis does not extend to the Paper-A-compliant realized-gain ceiling ($G \approx 6.6\times10^{17}$, Section~\ref{sec:kernels}); that ceiling lies further right along the same green region and moves $\rho^{*}$ by the factor of $\sim$2 quoted in Section~\ref{sec:crossover}.}
\label{fig:phasespace}
\end{figure}

The comparison with M is one line only under full seed fidelity and $p_s^{\rm seed} = p_s$. In that case Eq.~(\ref{eq:hvsmgeneral}) reduces to $(1-f)\,U_{\mathrm S} > (1-p_{d})\,U_{\mathrm M}$ at $p_d^{\mathrm M} = 1$ (a deliberating migrant credited at full fidelity); deep in migration-favourable territory ($U_{\mathrm M}/U_{\mathrm S} \approx 2.2\times10^{4}$ at $\rho+\lambda = 0.5\,\rho^{*}$) that requires $1 - p_{d} \lesssim 4\times10^{-5}$. The rescuing assumption, stated here because the dominance claim rests on it: a migrating lineage carries its own drift factor $p_{d}^{\mathrm M}$ over the same horizon, and a non-deliberating seed drifts no more than a deliberating lineage in transit ($p_{d} \geq p_{d}^{\mathrm M}$), whence Eq.~(\ref{eq:hvsmgeneral})'s right-hand side is bounded above by zero and $U_{\mathrm H} \geq U_{\mathrm M}$ term by term. Given that assumption, and given $p_s^{\rm seed} = p_s$ (Table~\ref{tab:params}: adopted, not derived, since seed transit hazards are unmodelled separately), the dominance itself is close to a bookkeeping identity; the substantive content of this section is the narrowness of the band beyond the threshold, $1.7\,\rho^{*}$ at $f = 10^{-6}$, given a nearly free seed. A seed with materially lower survival than the deliberating lineage ($p_s^{\rm seed} < p_s$) would need a correspondingly larger $p_d$ margin to compensate, per Eq.~(\ref{eq:hvsmgeneral}); this paper does not attempt that separate estimate.
...
Third, the loud residue does not vanish. Throughout this paper, loud denotes an expansion-front signature in the grabby-aliens sense (Section~\ref{sec:population}); localized waste heat from a stationary Matrioshka-class collector is a distinct residue class, treated on its own terms in Section~\ref{sec:residues}. Under pure M, migrated lineages leave planetary space entirely and the sky is silent. Under dominant H, the stay component continues local densification, whose Matrioshka waste heat is the classic detectable technosignature \citep{Dyson1960,Wright2014}. The hybrid therefore predicts both residue classes in linked proportion, the basis of the new population statistic in Section~\ref{sec:residues}.
...
\label{eq:gamma}
\end{equation}
which declines with horizon toward the minimum plausible rate: the patient scenarios dominate the expectation at long $t$. An agent placing equal weight on $\rho = 10^{-3}$ and $\rho = 10^{-6}$ has $\rho_{\rm eff}(\tau) = -\ln[\tfrac12(e^{-100} + e^{-0.1})]/\tau \approx 7.9\times10^{-6}$~yr$^{-1}$ at $\tau = 10^{5}$~yr (the certainty-equivalent rate approaches the $10^{-6}$ floor only at $t \gtrsim 10^{7}$~yr), still a factor of 25 below the threshold of Eq.~(\ref{eq:threshold}). Migration therefore requires only uncertainty about patience, not patience itself, which strengthens the case beyond the exponential baseline. For calibration in the other direction, empirical institutional discount rates cluster at $10^{-2}$--$10^{-3}$~yr$^{-1}$ (Weitzman's survey of professional economists' rate judgements spans this range and declines toward its lower end for far-future horizons; \citealt{Weitzman2001}), failing the threshold by up to two orders of magnitude: the framework describes old lineages, not present-day institutions.

\paragraph{Sail speed and the waiting alternative.} \citet{Heller2017} showed that a civilization expecting propulsion to improve faces an incentive trap: launching now can be dominated by waiting for a faster vehicle that overtakes the earlier one. In this framework the trap does not bind. Waiting costs discounted utility at the rate $\rho + \lambda$ on the entire deferred payoff, while speed improvements enter only through $\tau \propto 1/v$ inside the same energy economics that already caps beamed sails at $0.1$--$0.2c$; a bounded reduction in $\tau$ cannot repay an open-ended delay, and the seed option makes the launch cost too small for the saving to matter.
...
Paper B posed the question this paper answers: under what conditions does relocation to a thermodynamically privileged destination beat staying home and building? The answer has three parts, each built from stipulated inputs (destination multiplier $G$, transit survival $p_s$, seed cost $f$) rather than measured ones; what the analysis derives is their combination, not their values. The crossover between migration and densification is a patience threshold, $\rho + \lambda < \ln(G p_{s})/\tau$ (valid for $G p_s > 1$), whose logarithmic form makes it robust to the large uncertainties in the destination multiplier and transit risk; at fiducial, stipulated parameters it asks only for combined discount-plus-hazard half-lives beyond a few thousand years. Given a nearly free, stipulated seed cost and near-unity seed fidelity, the seed-and-stay hybrid dominates both pure strategies throughout the migration-favourable region; the result this paper derives, rather than stipulates, is the band's narrowness, extending only to roughly $1.7\times$ the threshold at $f = 10^{-6}$ before the exponential discount on the deferred payoff closes the window. Within that band the incomplete-compliance problem reduces to the joint distribution of patience and the optimization goal. And the hybrid's linked residues yield a population-level prediction, a formation-rate ratio of Matrioshka sources to engineered clusters of order unity, for which existing infrared nulls supply a joint-likelihood limit only after weighting by the maintenance lifetime of home installations, concentrating the family's surviving parameter space on the pure-migration and short-maintenance corners that the $\omega$~Cen campaign of Paper C tests directly.
