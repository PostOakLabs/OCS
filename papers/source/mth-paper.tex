...
\date{August 2026 \\[4pt] \small Draft v2.6 (last revised 2026-08-27) --- first paper of the set; companion to the inward-migration review (Paper B), the Omega Centauri campaign (Paper C),\\ \small the migration economics (Paper D), and the engineered-IMBH systems paper (Paper E);\\ \small prepared for omegacentauri.me}
...
central density $\rho_c \approx 4\times10^{3}\,\msun\,\mathrm{pc}^{-3}$,
consistent with the adopted value in Table~\ref{tab:ocparams} and with the
catalogued central luminosity density at the global mass-to-light ratio.
...
the Eddington-duration column assumes $\eta = 0.1$ feeding a
$2\times10^{4}\,\msun$ hole ($2{,}250$~yr per solar mass,
Section~\ref{sec:energy}).}
\label{tab:encmass}
\begin{tabular}{lccc}
\toprule
$r$ & $M(<r)$ & stars & Eddington duration \\
\midrule
$1$~pc & $1.6\times10^{4}\,\msun$ & $3.7\times10^{4}$ & $3.6\times10^{7}$~yr \\
$2$~pc & $1.1\times10^{5}\,\msun$ & $2.5\times10^{5}$ & $2.4\times10^{8}$~yr \\
$10$~pc & $1.9\times10^{6}\,\msun$ & $4.3\times10^{6}$ & $4.2\times10^{9}$~yr \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Phase 4 --- Spin-up.} Sustained prograde feeding ($\sim$$10^{4\text{--}5}$ stars over $\sim$$3.5\times10^{7}$~yr, $0.79$ Salpeter e-foldings to reach Thorne's radiation-limited $\astar = 0.998$ ($M_{f}/M_{i} \approx 2.2$) \citep{Bardeen1970,Thorne1974,Swanson2026Engineered}; Section~\ref{sec:energy}) drives $\astar \to 0.9$--$0.998$; the ISCO migrates inward by a factor $\sim$5 and jet efficiency crosses unity in MAD episodes; the fueled swarm holds its $\sim$$10^{2}\,r_g$ working boundary throughout \citep{Swanson2026Engineered}. \emph{Signature: episodic jet activity if imperfectly managed; high spin as a permanent record. Secular cluster-core depletion is a weaker observable than earlier drafts implied: the accreted $2.4\times10^{4}\,\msun$ (Section~\ref{sec:energy}) is $22.5$ per cent of the stellar mass inside 2~pc but only $1.29$ per cent of the mass inside the 10-pc reservoir the fuel census requires, recomputed directly against the enclosed-mass profile of Section~\ref{sec:encmass} (Table~\ref{tab:encmass}), the removed stellar mass reappears as central point mass so the total enclosed profile is preserved, and any deficit refills on the $\sim$4-Gyr core relaxation time \citep{Harris1996,Swanson2026Engineered}.} We flag the strongest hidden premise of the whole architecture here: Phases 3--5 presuppose institutional or goal stability over $10^{8}$~yr, longer than mammalian evolutionary history, and stellar feeding requires active orbit-shaping of individual stars with non-trivial delta-$v$ budgets. We know of no principled argument that either is achievable; the architecture is offered conditional on both.
...
\textbf{Beamed-momentum tugs are excluded by the waste-heat ceiling.} Imparting $\Delta v \sim 10^{2}$~km\,s$^{-1}$ to the $2.4\times10^{4}\,\msun$ that Phase 4 consumes costs $\sim$$2.4\times10^{44}$~J of kinetic work, $\sim$$6\times10^{2}\,L_\odot$ averaged over $3.5\times10^{7}$~yr, which fits inside the concealment budget with room to spare. Paying that momentum in photons does not: a photon tug needs $E = \Delta p\,c \simeq 1.4\times10^{48}$~J, or $\sim$$3.4\times10^{6}\,L_\odot$ sustained over the $3.5\times10^{7}$~yr episode, $4.5\times$ the ambient fuel ceiling (defined here as $7.6\times10^{5}\,L_\odot$, the JWST-limited accretion luminosity available to Phase 4 feeding; \citealt{Chen2025JWST,Swanson2026Engineered}) and two and a half to three and a half orders of magnitude above the mid-infrared limits that already bound an operating system at $\omega$~Cen \citep{Swanson2026Engineered}. The photons are absorbed by the star being moved and reradiated at stellar temperature across the cluster, so there is nowhere to put them. Gravitational-assist sequencing among cluster stars, which uses other stars as momentum reservoirs and pays only actuator overhead, is the one scheme that survives its own thermal signature.
