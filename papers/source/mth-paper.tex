...
\date{July 2026 \\[4pt] \small Draft v1.2 (last revised 2026-07-23) --- first paper of the set; companion to the inward-migration review (Paper B), the Omega Centauri campaign (Paper C),\\ \small the migration economics (Paper D), and the engineered-IMBH systems paper (Paper E);\\ \small prepared for omegacentauri.me}
...
\begin{table}[tbp]
\centering
\caption{The P2 selection criteria applied to the leading Galactic candidate systems. Columns: current IMBH evidence; inferred or claimed mass versus the preferred $10^{3}$--$10^{5}\,\msun$ range; core density and low-mass fuel reservoir; dynamical age and quiescence. Symbols: $++$ strong, $+$ favourable, $\circ$ neutral or contested, $-$ unfavourable.}
\label{tab:candidates}
\scriptsize
\begin{tabular}{p{1.9cm}cccc p{6.3cm}}
\toprule
System & Evidence & Mass & Fuel & Age/quiet & Notes and key references \\
\midrule
$\omega$~Cen & $++$ & $++$ & $++$ & $++$ & Fast-star kinematic lower bound contested by pulsar timing (Section~\ref{sec:tension}) \citep{Haberle2024Nature,BanaresHernandez2025} \\
47~Tuc & $\circ$ & $+$ & $++$ & $++$ & Pulsar-timing claim of $\sim$$2.3\times10^{3}\,\msun$ \citep{Kiziltan2017}; not required by later modelling \citep{Mann2019} \\
M54 & $+$ & $+$ & $+$ & $+$ & Nuclear cluster of the Sagittarius dwarf; kinematic hints at $\sim$$10^{4}\,\msun$ \citep{Greene2020}; 26~kpc distant \\
NGC~6388 & $\circ$ & $+$ & $+$ & $+$ & Kinematic claims disputed; deep radio nulls \citep{Tremou2018} \\
NGC~6441 & $\circ$ & $+$ & $+$ & $+$ & As NGC~6388; radio nulls \citep{Tremou2018} \\
Terzan~5 & $-$ & $\circ$ & $++$ & $\circ$ & Densest MSP population known, but heavy bulge extinction and no direct IMBH evidence \citep{Tremou2018} \\
M15 & $\circ$ & $\circ$ & $++$ & $++$ & Historical candidate; kinematics equally fit by a concentrated remnant population \citep{Baumgardt2019,Greene2020} \\
\bottomrule
\end{tabular}
\end{table}
...
The $\kb T_H \ln 2$ figure is the marginal cost of \emph{disposal at the horizon}; it is not the delivered cost from an orbiting platform, and the gap between the two can be estimated rather than merely flagged. Consider radiative delivery, the least demanding channel. A carrier photon must be aimable at the horizon, requiring wavelength $\lambda \lesssim r_g$; at $M = 2\times10^{4}\,\msun$, $r_g \simeq 3\times10^{7}$~m, so the minimum carrier energy is $E_\gamma \gtrsim hc/r_g \simeq 7\times10^{-33}$~J. If each such photon carries a single bit, the delivered cost per erased bit is $\sim$$7\times10^{-33}$~J, a factor $\sim$$4\times10^{9}$ below the CMB figure of $2.6\times10^{-23}$~J; denser coding is permitted up to the Bekenstein channel capacity \citep{Bekenstein1981,Bousso2002} but yields only modest further gains at this energy scale. Charging realistic engineering losses against the ideal (pointing and capture inefficiency, error-correction overhead, the fraction of carriers scattered rather than absorbed), a penalty of one to three orders of magnitude, leaves a defensible realized gain of $\sim$$10^{6}$--$10^{9}$ over the 2.7~K sky. Gravitational bookkeeping does not change this range: energy accounted at a platform of radius $r \gtrsim 10^{2}\,r_g$ is redshifted at infinity by the platform's lapse factor, an $O(1)$ Tolman correction at these radii. The realized economy therefore sits well above the $\kb T_H \ln 2$ floor and well below the CMB figure, and we quote $10^{6}$--$10^{9}$ rather than the floor-to-floor ratio of $\sim$$10^{12}$ wherever the realized gain is meant. An improvement of six orders of magnitude over the 2.7~K sky already dominates the aestivation trade.
...
\label{eq:dilation}
\end{equation}
\citep{BardeenPressTeukolsky1972}: $d\tau/dt = \sqrt{1/2} \simeq 0.707$ at the Schwarzschild ISCO, falling to $\simeq 0.093$ (a factor $\sim$11) at the prograde ISCO for $\astar = 0.998$ (Fig.~\ref{fig:isco}). Larger factors require powered non-geodesic hovering at diverging thrust cost and are not usable by stable platforms; $\sim$$11{:}1$ is the ceiling for stable circular orbits. A tiered swarm can therefore place archival and slow-integration processes deep (subjectively skipping across external time) and interactive processes high, with the choice of orbit acting as a clock-rate dial, a resource with no terrestrial analogue, though we note its strategic value (e.g.\ outwaiting external change) trades directly against responsiveness.
