...
\date{September 2026 \\[4pt] \small Draft v2.8 (last revised 2026-09-03) --- first paper of the set; companion to the inward-migration review (Paper B), the Omega Centauri campaign (Paper C),\\ \small the migration economics (Paper D), the engineered-IMBH systems paper (Paper E), the joint accretion bound (Paper F),\\ \small the residual X-ray census (Paper G), and the mass-tension paper (Paper H);\\ \small prepared for omegacentauri.me}
...
This paper develops a third member of that family, which we call the \emph{Macro Transcension Hypothesis} (MTH). Its core claim is that the inward trajectory identified by Smart has a concrete, macroscopic, observationally accessible destination in ordinary astrophysics: rapidly spinning massive black holes embedded in dense, dynamically old stellar systems. No new physics, no exit from the universe, and no multi-gigayear dormancy are required. The advantages that drive the migration (accretion power at up to $\sim$42 times the mass-energy efficiency of hydrogen fusion at the highest spin disk accretion reaches, an event horizon serving as an entropy sink of unlimited capacity, information storage at the Bekenstein--Hawking density, and gravitational time dilation as a controllable computational resource) are all available in the present epoch, and all follow from textbook general relativity and black-hole thermodynamics \citep{MTW1973,BardeenPressTeukolsky1972,Bekenstein1973,Bekenstein1974}. The hypothesis predicts that the most advanced civilizations remain present but electromagnetically invisible, because maximum computational efficiency, viewed from outside, is electromagnetically quiet.

The MTH would remain idle philosophy if it did not select targets. The selection criteria (a massive black hole of modest depth, intermediate rather than supermassive; a dense retinue of low-mass stars as fuel; dynamical age sufficient for any migrating civilization to have arrived and matured; and a quiescent electromagnetic environment) converge on a short list of Galactic systems, at the top of which sits Omega Centauri ($\omega$~Cen, NGC~5139): the Milky Way's most massive globular cluster, widely interpreted as the stripped nucleus of an accreted dwarf galaxy \citep{HilkerRichtler2000,BekkiTsujimoto2019,Ibata2019}, host to $\sim 10^{7}$ stars of mean age $12.08 \pm 0.01$~Gyr \citep{Clontz2024}, and, since the detection of seven fast-moving stars within its central 3~arcsec (five of which are robust detections), host to the strongest current candidate for an intermediate-mass black hole (IMBH) in the Galaxy \citep{Haberle2024Nature}. That candidate is simultaneously the subject of a sharp observational controversy: combined stellar-kinematic and millisecond-pulsar-timing analysis places a $3\sigma$ upper limit of $\sim$6,000\,\msun{} on any central point mass \citep{BanaresHernandez2025}, formally inconsistent with the kinematic lower bound. And it is electromagnetically dark to remarkable depth: neither $\sim$170 hours of ATCA radio integration \citep{Mahida2026}, nor JWST infrared photometry \citep{Chen2025JWST}, nor a $\sim$291~ks Chandra exposure \citep{Haggard2013} detects any accretion signature. Every element of this situation (a contested central mass, perfect electromagnetic silence, an imminent decisive instrument in LISA) makes $\omega$~Cen a natural laboratory for testing inward-migration hypotheses, entirely independent of whether the MTH is true.
...
\item \textbf{(Speculative hypothesis.)} If long-lived technological civilizations exist and even a subset optimizes for computational capacity \citep{Sandberg1999}, the environments of claim 1 act as attractors, and the resulting migration resolves the Fermi paradox without rare-Earth or doomsday assumptions. This is the MTH proper, stated formally in Section~\ref{sec:hypothesis}. We regard it as less parsimonious than the null hypothesis (technological life is rare or absent) and say so.
...
Section~\ref{sec:related} situates the MTH in the literature. Section~\ref{sec:hypothesis} states the hypothesis formally as four postulates. Section~\ref{sec:thermo} develops the quantitative thermodynamic and computational case. Section~\ref{sec:omegacen} reviews $\omega$~Cen and its IMBH evidence, including the current mass tension. Section~\ref{sec:falsification} presents the falsification framework and observational roadmap; the staged architecture that motivates it, sketched at the level of explicitly labelled speculation, is developed in Appendix~\ref{sec:architecture}. Section~\ref{sec:discussion} addresses objections, and Section~\ref{sec:conclusion} concludes. Figure~\ref{fig:flowchart} previews the argument chain and its epistemic register.

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{figs/fA_flowchart.pdf}
\caption{The argument chain of the paper as a left-to-right cascade, coloured by the epistemic register of Section~\ref{sec:claims}: green, the speculative postulates (P1, P3, P4); blue, the established physics of P2 (Section~\ref{sec:thermo}); orange, the selection theory (Section~\ref{sec:selection}); red, the falsification framework (Section~\ref{sec:falsification}, Table~\ref{tab:tests}). Each box carries the quantitative anchor developed in the section it points to. The observable residue P4 branches to the target application (Section~\ref{sec:omegacen}), and the framework terminates in the decision tree of Figure~\ref{fig:tree}.}
\label{fig:flowchart}
\end{figure}
...
\subsection{New content of the hypothesis}
...
\begin{description}
\item[P1 (Optimization pressure).] Among long-lived technological civilizations, a non-empty subset evolves (by competition, self-modification, or design) toward maximizing long-term computational capacity per unit of controlled resource, under at least one of three conditions on the objective: (i) temporal discounting, (ii) a requirement that the computation be unified and latency-bound, or (iii) a bounded planning horizon. The three are needed because an undiscounted maximizer of total integrated computation over unbounded time is served by expansion, for reasons set out in Section~\ref{sec:crossover}; they are stated here rather than added later. (Motivation: computation is the universal currency into which information-processing agents can convert essentially any terminal goal. This is the instrumental-convergence thesis \citep{Omohundro2008,Bostrom2012}, given formal treatment by \citet{BensonTilsenSoares2016}, and the same premise adopted by \citealt{Smart2012} and \citealt{Sandberg2016}. What remains open is the step from convergent instrumental goals in a single agent to a population claim over lineages; P1 asserts only a non-empty subset, the weakest form that step permits.)
...
\begin{enumerate}
\item \textbf{Black-hole mass in the intermediate range ($10^{3}$--$10^{5}\,\msun$).} Energy extraction (Section~\ref{sec:energy}) scales with available fuel and with Eddington-limited throughput ($\propto M$), favouring larger $M$. The lower bound is set by tides. The differential field at the Schwarzschild ISCO is $2GM/r_{\mathrm{ISCO}}^{3} = c^{6}/(108\,G^{2}M^{2})$, which is $\simeq 0.95$~s$^{-2}$ at $2\times10^{4}\,\msun$ and scales as $M^{-2}$; requiring a 10-m free-flying node to stay below $\sim$$10^{2}\,g$ of differential acceleration gives $M \gtrsim 2\times10^{3}\,\msun$, of which the stated $10^{3}$ floor is the round order of magnitude. The upper bound is not a cost that grows with $M$ but two considerations that cap the useful range: Bekenstein--Hawking storage already saturates any plausible need at $10^{4}\,\msun$ (Section~\ref{sec:storage}), so further mass buys nothing on that axis, and the environments in which holes above $10^{5}\,\msun$ are actually found are deep galactic-centre potentials crowded with disruptive astrophysics (Section~\ref{sec:whynotsgra}). The intermediate range secures every advantage at minimal exposure.
...
Milky Way globular clusters with massive-IMBH candidates satisfy all four criteria simultaneously; $\omega$~Cen satisfies them maximally (Section~\ref{sec:omegacen}). The Galactic Centre fails criterion 1 and 4; isolated stellar-mass black holes fail criterion 3; young massive clusters fail criterion 4. Retention is not the bottleneck: survival analyses of IMBHs in dense clusters find that holes above $\sim$$10^{3}\,\msun$ are generally retained against dynamical ejection over a Hubble time \citep{Martinez2026}, so if formation occurs at all, the P2 class is populated today. Figure~\ref{fig:massspin} locates the measured-spin stellar-mass holes, the IMBH candidates, and the P2 selection region in the mass--spin plane.

\begin{figure}[tbp]
\centering
\includegraphics[width=0.82\textwidth]{figs/fA_massspin.pdf}
\caption{The mass--spin plane. Blue: stellar-mass black holes with spin measurements (representative continuum-fitting values; compilation in \citealt{Reynolds2021}). Red hatched band: the contested $\omega$~Cen central mass, $8\times10^{3}$--$5\times10^{4}\,\msun$ (Table~\ref{tab:mass}), spin unknown. Grey arrows: IMBH candidates constrained only from above (47~Tuc, \citealt{Kiziltan2017}; M15, \citealt{Greene2020}). Grey open symbol: Sgr A* (spin inferred; \citealt{GRAVITY2022}). Green: the P2 selection region, $10^{3}$--$10^{5}\,\msun$ at $\astar \gtrsim 0.9$ (criteria 1--2). Dash-dotted line: the \citet{Thorne1974} spin-equilibrium limit $\astar = 0.998$. Dashed curve: schematic LISA EMRI mass reach at 5.49~kpc, anchored at the T4 threshold of $8.2\times10^{3}\,\msun$ (Table~\ref{tab:tests}) and drawn slightly deeper at high spin, where the inspiral spends more cycles in band. The curve states a requirement the test places on LISA, not a demonstrated reach at this distance; no mission-level sensitivity study for cluster EMRIs at 5.49~kpc is cited here, and the threshold should be read as the mass resolution T4 would need.}
\label{fig:massspin}
\end{figure}
...
\midrule
$\omega$~Cen & $++$ & $++$ & $++$ & $++$ & 5.5 & $\sim$$3\times10^{3}$ & 0.15 & $\geq 8.2\times10^{3}$ (kinematic) vs.\ $<6\times10^{3}$ (timing); contested (Section~\ref{sec:tension}) \citep{Haberle2024Nature,BanaresHernandez2025} \\
...
This section defends postulate P2 quantitatively. Throughout, $M$ is the black-hole mass, $\astar \equiv Jc/GM^{2} \in [0,1)$ the dimensionless spin, $r_{g} \equiv GM/c^{2}$ the gravitational radius, and we evaluate fiducial numbers at $M = 2\times10^{4}\,\msun$, the geometric midpoint of the kinematic side of the contested $\omega$~Cen range, $\sqrt{8.2\times10^{3} \times 5\times10^{4}} = 2.0\times10^{4}$ (Section~\ref{sec:omegacen}); the pulsar-timing side would place the midpoint lower, and we use the kinematic interval throughout so that the fiducial is stated rather than selected. All results in this section are standard physics; no claim about extraterrestrial intelligence is made until Appendix~\ref{sec:architecture}.
...
\label{eq:eta}
\end{equation}
which rises from $\eta = 1-\sqrt{8/9} \simeq 0.057$ at $\astar = 0$ ($r_{\mathrm{ISCO}} = 6\,r_g$) to $0.321$ (pure geodesic value; the photon-capture-corrected equilibrium efficiency is $\simeq 0.30$) at the \citet{Thorne1974} radiative spin-equilibrium limit $\astar = 0.998$, and formally to $1 - 1/\sqrt{3} \simeq 0.423$ as $\astar \to 1$ (Fig.~\ref{fig:efficiency}). Hydrogen fusion releases $0.71$\,per cent of the rest mass burned, but a real star offers only its hydrogen fraction $X_{\rm H} \simeq 0.7$ to that channel, so complete fusion of a star yields $\simeq 5\times10^{-3}$ of stellar mass; an unassisted main-sequence star processes roughly a tenth of its mass through its core over its lifetime, and only the hydrogen in that tenth burns, so a Dyson-type collector \citep{Dyson1960} captures a lifetime yield of order $5\times10^{-4}$. Both denominators therefore carry $X_{\rm H}$, which earlier drafts applied to the stellar yield alone. Per unit of fuel mass under the civilization's control, disk accretion onto a high-spin hole outperforms complete fusion of the same mass by a factor of $\sim$11 at $\astar = 0$ and $\sim$60 at the Thorne cap (formally $\sim$85 as $\astar \to 1$), and outperforms Dyson harvesting without star lifting by $\sim$115--600 (formally $\sim$850). Against the hydrogen-only convention used in earlier drafts the Dyson factors read $\sim$80--420 instead; the numerator is the same, and only the treatment of $X_{\rm H}$ in the denominator differs. The comparison set matters, because star lifting changes it: a civilization that strips the envelope and feeds the core accesses close to the full hydrogen inventory \citep{Criswell1985,Matloff2017,ScogginsKipping2023}, raising the yield toward the $5\times10^{-3}$ ceiling and collapsing the Dyson disadvantage to the same $\sim$11--60 as complete fusion. The entropy-sink and storage advantages (Sections~\ref{sec:landauer}--\ref{sec:storage}) are independent of which factor applies.
...
\addplot[thick, blue!60!black, mark=*, mark size=1.4pt] coordinates {
 ...
 (0.99,0.2641) (0.998,0.3210)
};
\addplot[thick, dashed, black!45, forget plot] coordinates {
 (0.998,0.3210) (0.9995,0.3597) (1.0,0.4226)
};
...
The $\kb T_H \ln 2$ figure is the marginal cost of \emph{disposal at the horizon}; it is not the delivered cost from an orbiting platform, and the gap between the two can be estimated rather than merely flagged. Consider radiative delivery, the least demanding channel. Efficient absorption requires a carrier wavelength $\lambda \lesssim r_g$, the geometric-optics condition for capture rather than diffraction around the hole (aiming is a separate and easier requirement, treated below). At $M = 2\times10^{4}\,\msun$, $r_g \simeq 3\times10^{7}$~m, so the vacuum-gravitational floor on the carrier energy is $E_\gamma \gtrsim hc/r_g \simeq 7\times10^{-33}$~J, corresponding to a frequency of $\sim$10~Hz. That floor is unreachable through plasma: propagation requires $\nu > \nu_p = 8.98\,\mathrm{kHz}\,\sqrt{n_e/\mathrm{cm}^{-3}}$, and even the cluster's ambient $n_e = 0.23\,\mathrm{cm}^{-3}$ (47~Tucanae analogy value; \citealt{Abbate2018}) gives $\nu_p \simeq 4.3$~kHz, while any fed inner-flow environment ($n_e \sim 10^{4}$--$10^{8}\,\mathrm{cm}^{-3}$ inside $\sim$$10^{2}\,r_g$) gives $\nu_p \simeq 1$--$90$~MHz. The minimum propagating carrier energy $E = h\nu_p$ therefore spans $2.9\times10^{-30}$~J at the ambient density to $6.0\times10^{-26}$~J at the densest fed flow, 2.6 to 6.9 decades above $hc/r_g$. Two consequences follow. First, single-bit-per-photon delivery no longer suffices: at the plasma-cutoff carrier energies the single-bit advantage over the CMB would erode to $9\times10^{6}$ in the ambient medium and to $4\times10^{2}$ in the densest fed flow considered. What preserves the realized range is dense coding: a carrier may bear many bits, up to the Bekenstein channel capacity \citep{Bekenstein1981,Bousso2002}, and coding at that capacity restores the delivered cost per bit to a factor of $10^{6}$--$10^{9}$ below the CMB figure of $2.6\times10^{-23}$~J.

This requirement can be stated as a length rather than assumed. The Bekenstein bound caps the information carried by a packet of energy $E$ confined to radius $R$ at $I \leq 2\pi ER/(\hbar c \ln 2)$ \citep{Bekenstein1981}, so a diffraction-limited packet one wavelength long carries at most $2\pi h/(\hbar \ln 2) \simeq 57$ bits whatever its frequency. Reaching a realized gain $G$ against the CMB figure requires carriers averaging $b = G E_{\rm carr}/(\kb T_\gamma \ln 2)$ bits, so the packet coherence length the coding demands is $L = Ghc/(57\,\kb T_\gamma \ln 2)$, the same at every carrier floor: about $1.4\times10^{2}$~m at the bottom of the claimed range and $1.4\times10^{5}$~m at its top. Two properties of that expression should be stated plainly, because earlier drafts stated the second one backwards. First, $E_{\rm carr}$ cancels, which is why the length is common to all four floors. Second, these are the lengths at which the packet sits exactly on the Bekenstein bound: at every $G$ and every floor the ratio $b/N_\lambda$ comes out at $57$ bits per wavelength exactly. There is no headroom anywhere in the range, and a code running at a fraction $f$ of capacity needs a packet longer by $1/f$. What the assumption consumes is packet coherence rather than any exotic code, and the quantity to check is therefore the length, not a fraction of capacity.

Delivery losses enter the same way, since a loss of throughput $f$ and a code rate $f$ are the same multiplier on the required length. Table~\ref{tab:delivery} prices the losses the preceding paragraphs name, and reports the $G$ that follows rather than assuming it. Under an optimistic budget the range is reached at $0.19$--$190$~km of coherence; under a conservative one at $3.0$--$3.0\times10^{3}$~km. Even the worst corner sits $\sim$$10^{3}$ times inside the $10^{2}\,r_g$ working boundary of $3\times10^{6}$~km, so the $10^{6}$--$10^{9}$ range survives being priced. What does not survive is the earlier claim that it is reached with capacity to spare.

\begin{table}[tbp]
\centering
\caption{Delivery loss budget for the radiative channel, and the packet
coherence length each budget requires. The losses are the ones named in the
text: error-correction overhead, capture and pointing efficiency into the
$6.75\times10^{-4}$ solid-angle target, transmission through the swarm
without re-interception, and the $\approx$1 per cent lapse correction at
$10^{2}\,r_g$. Throughput is their product; required length scales as its
inverse. The final column fixes the length at 100~km and reports the
realized gain that follows.}
\label{tab:delivery}
\footnotesize
\begin{tabular}{lccccccc}
\toprule
Budget & Code & Capture & Swarm & Lapse & Through- & \multicolumn{1}{c}{$L$ for} & $G$ at \\
 & rate & $f_{\rm sink}$ & transm. & & put & \multicolumn{1}{c}{$G=10^{6}/10^{9}$} & $L=100$~km \\
\midrule
Lossless (bound) & 1.00 & 1.00 & 1.00 & 1.00 & 1.000 & 0.135 / 135~km & $7.4\times10^{8}$ \\
Optimistic & 0.90 & 0.90 & 0.90 & 0.99 & 0.722 & 0.19 / 190~km & $5.4\times10^{8}$ \\
Conservative & 0.50 & 0.30 & 0.30 & 0.99 & 0.045 & 3.0 / $3.0\times10^{3}$~km & $3.3\times10^{7}$ \\
\bottomrule
\end{tabular}
\end{table}

Matter carriers relax the coherence constraint, at the price of an accounting question the earlier draft did not face. A one-gram pellet at $100$~km\,s$^{-1}$, confined to $1$~cm, has a Bekenstein capacity of $2.6\times10^{38}$ bits. Charged with kinetic energy alone it delivers at $1.9\times10^{-32}$~J per bit, a gain of $1.3\times10^{9}$; but the capacity in the numerator is set by the pellet's total energy, of which rest mass is $1.8\times10^{7}$ times the kinetic part, so charging only the kinetic term is not like-for-like. Charging the rest energy symmetrically gives a gain of $74$. Charging instead the opportunity cost of the rest mass, $\eta mc^{2}$ at $\eta = 0.30$, which is what the civilization forgoes by spending the pellet as a carrier rather than as fuel, gives $247$. We quote the matter channel at $\sim$$10^{2}$ and take the $10^{9}$ top of the delivered range from the radiative channel alone.

Second, matter carriers evade the cutoff entirely: cold mass dropped down the capture cone crosses the horizon with its entropy and energy aboard and no dispersion relation to satisfy, and serves as the fallback channel wherever the local plasma forbids cheap photons. Charging the engineering losses of Table~\ref{tab:delivery} against the radiative channel leaves a defensible realized gain of $\sim$$10^{6}$--$10^{9}$ over the 2.7~K sky, at the coherence lengths tabulated there. Gravitational bookkeeping does not change this range: energy accounted at a platform of radius $r \gtrsim 10^{2}\,r_g$ is redshifted at infinity by the platform's lapse factor, a $\approx$1 per cent correction at these radii. The realized economy therefore sits well above the $\kb T_H \ln 2$ floor and well below the CMB figure, and we quote $10^{6}$--$10^{9}$ rather than the floor-to-floor ratio of $\sim$$10^{12}$ wherever the realized gain is meant. An improvement of six orders of magnitude over the 2.7~K sky already dominates the aestivation trade.
...
The IMBH wins decisively on three of the four P2 axes and loses on the fourth. Storage scales as $\sum M_i^{2}$, so fragmenting the central mass into $2.5\times10^{4}$ holes of $10\,\msun$ costs a factor of 160 even against the twelvefold mass advantage. Sink depth scales as $T_H \propto M^{-1}$, a $2000\times$ edge for the IMBH, and the edge survives the delivery-level accounting of Section~\ref{sec:landauer}: the realized erasure economy tracks the delivered carrier cost rather than $T_H$ directly, and the vacuum carrier floor $hc/r_g$ is $1.3\times10^{-29}$~J for a $10\,\msun$ hole against $6.7\times10^{-33}$~J for the IMBH, the same $\sim$$2\times10^{3}$ ratio. The ISCO tidal field, $\propto M^{-2}$ at fixed $r/r_g$, is $4\times10^{6}$ times harsher at $10\,\msun$, excluding all but dust-scale hardware from the deep orbits where the dilation and efficiency gains live. And a computation distributed over the swarm pays parsec-scale light lag between nodes, against milliseconds across a single working orbit, failing the latency-bound condition of Section~\ref{sec:crossover} by construction. A computation-maximizer offered both configurations takes the single hole; consolidation is worth a large premium under the same objective.

The premium is a statement about preference, not about reach, and the gap between the two bounds what T3 can carry. Consolidating a remnant swarm means merging stellar-mass holes, and merger products receive gravitational-wave recoil kicks of $10^{2}$--$10^{3}$~km\,s$^{-1}$ for generic spin orientations \citep{Campanelli2007,Lousto2011}, against a central escape speed of some tens of km\,s$^{-1}$ in $\omega$~Cen. Consolidation by merger therefore ejects most of its own products, and we know of no mechanism by which a civilization could assemble $2.5\times10^{4}$ holes into one while retaining them. The subsection establishes that a maximizer would prefer one hole. It does not establish that one could be built.

This derivation does real work in the falsification program. T3 (Table~\ref{tab:tests}) treats a LISA-resolved remnant swarm as a decisive falsifier, and this subsection is why: the swarm loses on storage, sink depth, and tides by two to six orders of magnitude, so a maximizer with gigayears of run of the cluster would not have left the central mass in that configuration. T3 is thereby a derived kill switch rather than a stipulated one, conditional on the preference argument above transferring to a reachability claim, which the recoil problem leaves open. A resolved swarm is decisive against $H_2$ only if a maximizer with gigayears of run of the cluster could have consolidated it; if consolidation is dynamically blocked, a swarm is evidence about the cluster's formation history, not about whether anyone was there.
...
Central density (catalogue) & $\sim 3\times10^{3}\,\msun\,\mathrm{pc}^{-3}$ & \citet{BaumgardtVasiliev2021}; cf.\ $\sim 10^{3}$ in \citet{PryorMeylan1993} \\
Central density (King model, adopted) & $4.0\times10^{3}\,\msun\,\mathrm{pc}^{-3}$ & Section~\ref{sec:encmass}; used for every enclosed-mass figure \\
Declination & $-47.5^{\circ}$ & \citet{Harris1996} \\
\bottomrule
\end{tabular}
\end{table}

See Paper F App.~A for the shared-parameter table across the set, which cross-references this table's distance, cluster mass, and central-density rows against the other papers' own choices.

\subsection{The enclosed-mass profile}
\label{sec:encmass}

The fuel-census figures used in this paper (Sections~\ref{sec:selection}
and~\ref{sec:architecture}) are referred throughout to one stellar
enclosed-mass profile, stated here so that every ratio is reconstructible.
We integrate a single-mass lowered-isothermal (King 1962) model to the
catalogued concentration $c = \log_{10}(r_t/r_c) = 1.31$ and core radius
$r_c = 2.37'$ \citep{Harris1996}, scaled at our adopted distance
(Table~\ref{tab:ocparams}), and normalize to the total cluster mass
$4\times10^{6}\,\msun$. This gives $W_0 = 6.22$, $r_t = 77$~pc, and a
central density $\rho_c = 4.03\times10^{3}\,\msun\,\mathrm{pc}^{-3}$.
That is 34 per cent above the catalogued $\sim$$3\times10^{3}$ of
Table~\ref{tab:ocparams}, and the two are not interchangeable: a flat-core
estimate of $M(<1\,\mathrm{pc})$ lands within 6 per cent of the integrated
profile at $4.0\times10^{3}$ and 20 per cent low at $3\times10^{3}$. Every
enclosed-mass, fuel-census and depletion figure in this paper is referred to
the model value, and we state it once here rather than quoting the two as
though they agreed; the catalogued value is retained in
Table~\ref{tab:ocparams} as the external anchor the model is scaled against
through $c$ and $r_c$, not through $\rho_c$.
The model's projected half-mass radius, $8.3$~pc ($5.2'$), matches the
catalogued $5.00'$ to 3--4 per cent. Enclosed stellar mass:

\begin{table}[tbp]
\centering
\small
\caption{Stellar enclosed mass from the King profile of
Section~\ref{sec:encmass}. Star counts convert at $0.43\,\msun$ per star.
The two duration columns answer different questions and earlier drafts
printed only the first. Column 4 expresses the reservoir as Eddington-years
at a hole held fixed at $2\times10^{4}\,\msun$ ($2{,}250$~yr per solar mass,
Section~\ref{sec:energy}); it is linear in fuel mass and states the reservoir size; it is not an elapsed time. Column 5 is the elapsed time to
actually consume the reservoir, $t_{\rm S}\ln(1 + M_{\rm fuel}/M_i)$ with
$t_{\rm S} = \eta c^{2}/(L_{\rm Edd}/M) = 4.5\times10^{7}$~yr at
$\eta = 0.1$, which accounts for the Eddington limit rising as the hole
grows. The 2-pc and 10-pc rows are bookkeeping rather than scenarios: their
endpoint masses, $1.3\times10^{5}$ and $1.9\times10^{6}\,\msun$, leave the
$10^{3}$--$10^{5}\,\msun$ range of selection criterion~1, so a hole that
consumed either reservoir would no longer be the object this paper
selects.}
\label{tab:encmass}
\footnotesize
\begin{tabular}{lccccc}
\toprule
$r$ & $M(<r)$ & stars & Fuel at fixed $M$ & Elapsed & Endpoint $M$ \\
 & ($\msun$) & & (Eddington-yr) & (growth-aware) & ($\msun$) \\
\midrule
$1$~pc & $1.6\times10^{4}$ & $3.7\times10^{4}$ & $3.6\times10^{7}$ & $2.6\times10^{7}$~yr & $3.6\times10^{4}$ \\
$2$~pc & $1.1\times10^{5}$ & $2.5\times10^{5}$ & $2.4\times10^{8}$ & $8.3\times10^{7}$~yr & $1.3\times10^{5}$ \\
$10$~pc & $1.9\times10^{6}$ & $4.3\times10^{6}$ & $4.2\times10^{9}$ & $2.0\times10^{8}$~yr & $1.9\times10^{6}$ \\
\bottomrule
\end{tabular}
\end{table}

The profile is a stellar background and carries no mass segregation; a
multi-mass treatment is not needed for any figure quoted in this paper.


\subsection{The IMBH candidate and the mass tension}
\label{sec:tension}

Evidence for a central dark mass in $\omega$~Cen has accumulated for two decades, from integrated kinematics \citep{Noyola2008} through proper-motion modelling that initially yielded only upper limits at the $\sim$$10^{4}\,\msun$ level \citep{vanderMarelAnderson2010}, with stellar-mass black-hole subsystems long advanced as the alternative explanation \citep{Zocchi2019,BreenHeggie2013} and a systematic $N$-body survey concluding that neither $\omega$~Cen nor NGC~6624 required an IMBH \citep{Baumgardt2019}. The situation changed qualitatively when \citet{Haberle2024Nature}, using a proper-motion catalogue of 1.4 million stars built from over 500 HST epochs \citep{Haberle2024oMEGACatII}, identified seven stars within 3~arcsec of the cluster centre moving faster than the local escape velocity, of which five are robust detections. Their velocities on those five stars alone require a point mass $\geq 8{,}200\,\msun$; including acceleration limits on the same stars raises the lower bound to $21{,}100\,\msun$ at 99\,per cent confidence (main text of \citealt{Haberle2024Nature}).\footnote{The abstract of \citet{Haberle2024Nature} quotes only the velocity-only $8{,}200\,\msun$ limit; the $21{,}100\,\msun$ figure appears in the main text. A Nature \emph{Author Correction} (doi:10.1038/s41586-024-08017-4) fixes an equation typo and a right-ascension offset; the results are unaffected.} Independent $N$-body modelling finds that an IMBH grown in situ to $\sim$$5\times10^{4}\,\msun$ reproduces the present-day cluster structure and fast-star population \citep{GonzalezPrieto2025}.
...
\midrule
Lower bound (velocities only) & $\geq 8{,}200\,\msun$ & 5 of 7 fast stars (robust subset), HST proper motions \citep{Haberle2024Nature} \\
...
\item[$H_2$ (MTH).] An IMBH exists and is, or has been, the substrate of a migrated civilization (Appendix~\ref{sec:architecture}).
\end{description}

\noindent
Four load-bearing numbers below are taken from the companion infrastructure analysis \citep{Swanson2026Engineered}: the T2 waste-heat floor $L_{\rm waste} \gtrsim 10^{-4}P_{\rm comp}$, the Phase-3 erosion boundary, the $\sim$$4\times10^{11}$~yr Blandford--Znajek spindown time behind the T5 natal-spin argument, and the $7.6\times10^{5}\,L_\odot$ ambient Bondi ceiling used in Appendix~\ref{sec:architecture}. That companion is a preprint and is not yet refereed. We mark the thresholds that depend on it provisional, and a reader who declines the companion should treat T2 and T5 as stated without numerical thresholds.
...
\label{tab:tests}
\scriptsize
\begin{tabular}{p{0.6cm}p{3.1cm}p{3.1cm}p{2.5cm}p{1.6cm}p{2.5cm}}
\toprule
\# & Observation & Threshold & Instrument & Timeline & Kills \\
\midrule
T1 & Accretion luminosity from core (limit-tightening) & Current best mid-infrared bolometric-equivalent limit $L_{\rm acc} \approx 2.5\times10^{26}$~W ($\approx 10^{-9}L_{\rm Edd}$) \citep{Chen2025JWST}, versus the deeper band-limited Chandra X-ray bound $L_X(0.5$--$7.0\,\mathrm{keV}) \lesssim 1.6\times10^{23}$~W ($\lesssim 10^{-12}L_{\rm Edd}$) \citep{Haggard2013}; near-term target one further decade, $\lesssim 2.5\times10^{25}$~W. Supplementary trigger: sustained ($>1$~yr) brightening to $L_{\rm acc} > 10^{29}$~W & JWST MIRI; ATCA/ MeerKAT; Chandra & now--2030 & Constraint channel, not a kill: each deepening null lowers the ceiling on managed accretion throughput without retiring any phase. A brightening detection kills Phase 5 (managed environment). Bounds the accretion SED itself, where T2 bounds reradiated waste heat \citep{Haggard2013,Mahida2026} \\
T2 & Mid-IR waste-heat excess & $>3\sigma$ over stellar-population model, 10--25.5~$\mu$m (MIRI imaging limit); tested against the horizon-sink floor $L_{\rm waste} \gtrsim 10^{-4} P_{\rm comp}$ \citep{Swanson2026Engineered} & JWST MIRI & now--2030 & Constraint channel, not a kill: any Dyson-type capture is excluded, and a deepening null tightens the ceiling on Phase 4--5 throughput (currently $P_{\rm comp} \lesssim 10^{3\text{--}4}\,L_\odot$). Because the horizon-sink floor is $L_{\rm waste} \gtrsim 10^{-4}P_{\rm comp}$, no null retires managed operation outright; it drives $P_{\rm comp}$ down without bound \\
...
T4 & EMRI point mass below the kinematic floor (demotion, not kill) & clean chirp, $M < 8.2\times10^{3}\,\msun$ at $>3\sigma$; the trigger is the \citet{Haberle2024Nature} kinematic lower bound, not the $10^{3}\,\msun$ class edge & LISA & late 2030s--2040s & Demotes the selection ranking of Section~\ref{sec:selection}: $\omega$~Cen would no longer be a P2-optimal destination, though an $8\times10^{3}\,\msun$ hole still delivers $\sim 1.0\times10^{35}$~W (Eq.~\ref{eq:edd}), so $H_2$ is weakened rather than excluded \\
...
T6 & High-energy neutrino bursts from core & $\geq 3$ tracks within $1^{\circ}$ in $10^{2\text{--}3}$~s, $E \gtrsim 10$~TeV, $>5\sigma$ post-trials, no astrophysical counterpart & KM3NeT/ARCA ($\omega$~Cen is up-going from the Mediterranean; dec.\ $-47^{\circ}$) & 2026--2035 & Constraint channel, not a kill: a null at full sensitivity closes the \citet{Dvali2023} channel only, which the MTH does not require; a detection is consistent with $H_2$ without proving it \\
\bottomrule
\end{tabular}
\end{table}

The matrix divides into two kinds of test, and the division is not the one an earlier draft implied. T1, T2 and T6 are constraint channels: since $H_0$ and $H_2$ predict identical electromagnetic appearances, a deeper null in any of them lowers a ceiling on managed throughput and retires no phase, and there is no null threshold at which they adjudicate between the hypotheses. We state the consequence rather than leave it implicit: no accumulation of electromagnetic non-detections, however deep, can falsify the MTH. T3 and T5 are the decisive tests, T4 a ranking demotion, and all three are contingent on a compact-object inspiral occurring in LISA band during the mission. The MTH cannot survive a confirmed remnant swarm or a non-spinning IMBH, except as vacuous speculation about other systems; but whether it is adjudicated at all in this generation depends on an event rate the paper cannot pin down (Section~\ref{sec:tension}). Second, the LISA spin measurement is among the most informative numbers obtainable this generation (if an in-band inspiral delivers it; Section~\ref{sec:tension}), though both the natural-spin null and the expected yield must be stated carefully. Two natural channels are on the table for $\omega$~Cen, and earlier drafts adopted both at once, which left the high-spin branch with no discriminating power in either direction. We adopt one. The paper's own account of $\omega$~Cen's origin (Section~\ref{sec:omegacen}) is a gas-rich, accreted dwarf-galaxy nucleus, so the prior we carry is coherent disk growth, which generically reaches high spin \citep{BertiVolonteri2008,Reynolds2021}; because Blandford--Znajek spindown at ambient fueling operates on $\sim$$4\times10^{11}$~yr timescales \citep{Swanson2026Engineered}, such natal spin survives to the present. Under that prior a measured $\astar \gtrsim 0.9$ is expected on $H_0$ as well as on $H_2$ and is not by itself a residue. The alternative channels sharpen T5 rather than blunt it. Runaway stellar collisions build IMBHs of several $10^{3}\,\msun$ \citep{Fujii2024}, though those simulations do not report a remnant spin, so the channel leaves $\astar$ unconstrained, not low. Growth by randomly oriented accretion episodes drives the spin to an equilibrium near $\astar \approx 0.1$--$0.3$ with fluctuations of $\pm 0.2$ \citep{KingPringleHofmann2008,BertiVolonteri2008}, which places substantial prior weight at and below the T5 trigger $\astar < 0.1$, not in the small tail an earlier draft assumed. What we cannot supply is the relative weight of the coherent and chaotic channels for this object, and the quantity the test turns on is therefore that weight, which we cannot supply. A measured $\astar \gtrsim 0.9$ counts as supporting evidence only in conjunction: the discriminating observation is the conjunction of high spin, electromagnetic silence far below the ambient Bondi supply, and the regulation signature of T1 (P4a--b together). EMRI parameter estimation at LISA delivers spin to better than percent precision for favourable systems \citep{Babak2017,AmaroSeoane2018}. Third, the program is cheap: T1--T2 are archival or piggyback analyses; T6 requires only a pointed monitoring program on an instrument already built for the southern sky \citep{AdrianMartinez2016,KM3NeT2025}, following the high-energy SETI logic of \citet{Lacki2025} and the pipeline practices of standard point-source searches \citep{IceCubePS2020}; the partially built ARCA array has already demonstrated discovery-class capability with the $\sim$220-PeV event KM3-230213A, the most energetic neutrino yet observed \citep{KM3NeT2025}; and T3--T5 are free by-products of LISA's planned EMRI science \citep{Colpi2024}.
...
\node[o] (small) at (-5.1,-4.6) {$M < 8.2\times10^{3}\,\msun$:\\ \textbf{selection ranking demoted} (T4); $\omega$~Cen no longer P2-optimal};
\node[q] (spin) at (0,-4.6) {$M \geq 8.2\times10^{3}\,\msun$:\\ measure spin $\astar$};
...
\node[live] (highspin) at (5.1,-7.0) {$\astar \geq 0.9$ on a gas-starved hole:\\ \textbf{not discriminating on spin alone}; supporting only with silence + regulation; escalate T6 campaign};
...
\draw[arr] (emri) -- (small) node[lab, midway] {$M\!<\!8.2\!\times\!10^{3}$};
\draw[arr] (emri) -- (spin) node[lab, midway] {$M\!\geq\!8.2\!\times\!10^{3}$};
...
\end{tikzpicture}
\caption{Decision tree for the decisive (gravitational-wave) branch of the falsification program. Red boxes terminate the hypothesis or its exploitation model; the single green branch is the only outcome in which the MTH gains material support, and even there the inference is to ``anomaly requiring explanation,'' not detection. The mass branches are exhaustive at the $8.2\times10^{3}\,\msun$ kinematic lower bound of \citet{Haberle2024Nature}, which is the threshold T4 uses; an earlier version split at $5\times10^{3}$ on one side and $10^{4}$ on the other, leaving measured masses in $[5\times10^{3}, 10^{4})$, the interval containing the kinematic bound itself, with no terminal. Under the coherent-growth prior adopted in the text the high-spin branch is expected on $H_0$ too, which is why it is labelled as non-discriminating on spin alone.}
\label{fig:tree}
\end{figure}

\subsection{Supporting evidence criteria}

Symmetry requires stating the confirmation side. The MTH gains support only from conjunctions that $H_0$ finds awkward: (i) a LISA-confirmed IMBH with $\astar \gtrsim 0.9$ in conjunction with Gyr-scale gas starvation and the regulation signature of T1, since high spin alone admits the natal-disk explanation above; (ii) repeated $>5\sigma$ neutrino bursts from the core with hard spectra and no electromagnetic counterpart, matching the \citet{Dvali2023} phenomenology; or (iii) a measured low-mass mass-function slope in the central 0.1~pc lying below the \citet{GonzalezPrieto2025} model envelope at $>3\sigma$ (oMEGACat photometry reaches the relevant depth), the concrete form of the P4c depletion residue. Criterion (iii) is the weakest of the three and we mark it provisional. A $1.3$ per cent removal from the 10-pc reservoir preserves the total enclosed mass and is only claimed to be observable because it alters the stellar mass function rather than the density profile; whether a removal of that size imprints a $>3\sigma$ slope anomaly that survives $\sim$12~Gyr of mixing has not been forward-modelled, and until an $N$-body calculation supplies the predicted amplitude, (iii) states where to look without stating what to expect. No single line, including all three jointly, would constitute proof of technology; they would constitute an anomaly stack justifying escalated scrutiny. Conversely, we commit in advance: if LISA delivers T3, T4, or T5, the $\omega$~Cen application of the MTH should be retired without special pleading.
...
\paragraph{Scope of the $\omega$~Cen bet.} The MTH is a selection theory over a class; $\omega$~Cen is its highest-ranked accessible member, not its definition. Falsification at $\omega$~Cen (T3, or the ranking demotion of T4) retires the flagship application and substantially weakens the hypothesis (the Galaxy's best candidate failing is evidence about the class), but formally the theory survives in 47~Tuc, M54, and extragalactic nuclear clusters. We accept the methodological hazard this creates (unfalsifiability by retreat) and therefore bind ourselves to the class-level prediction: \emph{if no Galactic globular-cluster IMBH with $\astar \gtrsim 0.9$ exists, the MTH is wrong for the Milky Way}. Stating that in deliverable form matters, because a spin measurement requires an inspiral and the per-cluster EMRI rate is uncertain by orders of magnitude (Section~\ref{sec:tension}). The settleable version is conditional on yield: if LISA returns spin measurements for $\geq$3 Galactic globular-cluster IMBHs over its mission and none has $\astar \gtrsim 0.9$, we treat the class prediction as failed. A mission that returns fewer than three leaves the class claim unadjudicated rather than supported, and we commit in advance to reporting it that way, not to reading a null yield as survival.
...
% =====================================================================
\section{Staged exploitation architecture}
...
\textbf{Phase 4 --- Spin-up.} Sustained prograde feeding ($\sim$$10^{4\text{--}5}$ stars over $3.4$--$5.3\times10^{7}$~yr, $0.79$ Salpeter e-foldings to reach Thorne's radiation-limited $\astar = 0.998$ ($M_{f}/M_{i} \approx 2.2$) \citep{Bardeen1970,Thorne1974,Swanson2026Engineered}; Section~\ref{sec:energy}) drives $\astar \to 0.9$--$0.998$; the ISCO migrates inward by a factor $\sim$5 and jet efficiency crosses unity in MAD episodes; the fueled swarm holds its $\sim$$10^{2}\,r_g$ working boundary throughout \citep{Swanson2026Engineered}. \emph{Signature: episodic jet activity if imperfectly managed; high spin as a permanent record. Secular cluster-core depletion is a weaker observable than earlier drafts implied: the accreted $2.4\times10^{4}\,\msun$ (Section~\ref{sec:energy}) is $22.5$ per cent of the stellar mass inside 2~pc but only $1.29$ per cent of the mass inside the 10-pc reservoir the fuel census requires, recomputed directly against the enclosed-mass profile of Section~\ref{sec:encmass} (Table~\ref{tab:encmass}), the removed stellar mass reappears as central point mass so the total enclosed mass profile is preserved. The stellar mass function is not, and that is the observable P4c names. The refill clock was previously quoted as the $\sim$4-Gyr core relaxation time; that is the right clock for the 2-pc deficit ($\log t_{rc} = 9.60$) and the wrong one for the 10-pc reservoir the fuel census actually requires, whose half-mass relaxation time is $\log t_{rh} = 10.09$, or $\sim$12~Gyr \citep{Harris1996}. The 10-pc deficit is therefore more durable than earlier drafts allowed, on the order of the cluster's age rather than a third of it, and the correction runs against the hedge we had drawn from it \citep{Swanson2026Engineered}.} We flag the strongest hidden premise of the whole architecture here: Phases 3--5 presuppose institutional or goal stability over $10^{8}$~yr, longer than mammalian evolutionary history, and stellar feeding requires active orbit-shaping of individual stars with non-trivial delta-$v$ budgets. We know of no principled argument that either is achievable; the architecture is offered conditional on both.
...
\textbf{Natural refilling does not supply the rate.} Phase 4 needs $10^{4\text{--}5}$ stars over $3.4$--$5.3\times10^{7}$~yr, i.e.\ $\sim$$1$--$2\times10^{-3}$ stars yr$^{-1}$ for the $5.6\times10^{4}$ stars the mass budget implies. Natural loss-cone refilling, which resonant relaxation drives near the influence radius \citep{RauchTremaine1996}, delivers stars to the hole at the tidal-disruption rate, $\sim$$10^{-8}$--$10^{-7}$~yr$^{-1}$ for IMBHs in evolved clusters and $\sim$$5\times10^{-8}$~yr$^{-1}$ in the $\omega$~Cen models of \citet{GonzalezPrieto2025} specifically \citep{Swanson2026Engineered}. The shortfall is four to five orders of magnitude, and the repaired episode duration of Section~\ref{sec:energy} moves it by less than a factor of two. The natural channel is not a slower alternative to engineered delivery; it is not a fuel supply for this architecture at all, and essentially the whole Phase-4 budget must be actuated.

\textbf{Beamed-momentum tugs are excluded, on the cluster's own light rather than on the fuel ceiling.} The kinetic work is not the problem. Imparting $\Delta v$ to the $2.4\times10^{4}\,\msun$ that Phase 4 consumes costs $\tfrac{1}{2}M\Delta v^{2}$, which averaged over the $5.3\times10^{7}$~yr episode of Section~\ref{sec:energy} is $15\,L_\odot$ at $20$~km\,s$^{-1}$ and $3.7\times10^{2}\,L_\odot$ at $10^{2}$~km\,s$^{-1}$. Paying the same momentum in photons costs $E = \Delta p\,c$, larger by $2c/\Delta v$, which is a factor of $3\times10^{4}$ at $20$~km\,s$^{-1}$.

Earlier drafts priced that against a ceiling of $7.6\times10^{5}\,L_\odot$ described as ``the JWST-limited accretion luminosity''. Two corrections are needed. First the label: $7.6\times10^{5}\,L_\odot$ is the ambient Bondi accretion power at the fiducial mass and spin, taken from the companion infrastructure analysis \citep{Swanson2026Engineered}; the JWST limit of test T1 is $2.5\times10^{26}$~W, which is $0.65\,L_\odot$, and the two differ by a factor of $10^{6}$ because they are different quantities. Second the verdict. Against the Bondi ceiling the exclusion is not robust: at the $10^{2}$~km\,s$^{-1}$ earlier drafts assumed, the photon tug costs $2.2\times10^{6}\,L_\odot$, or $2.9\times$ the ceiling, but the impulse this section itself calls for is of order the local orbital speed, and the King profile of Section~\ref{sec:encmass} gives $8.2$, $15.2$ and $28.3$~km\,s$^{-1}$ at 1, 2 and 10~pc. At $20$~km\,s$^{-1}$ the photon tug costs $4.5\times10^{5}\,L_\odot$, which is $0.59$ of the ceiling and therefore fits; the exclusion threshold sits at $\Delta v \gtrsim 34$~km\,s$^{-1}$, above the whole orbital-speed bracket.

The exclusion survives on a comparator that does not depend on the contested ceiling. The photons are absorbed by the star being moved and reradiated at stellar temperature, so they are added to the cluster's integrated light, and $\omega$~Cen radiates $1.09\times10^{6}\,L_\odot$ in total at $M_{V,t} = -10.26$. The photon tug costs 17 per cent of that at $8.2$~km\,s$^{-1}$, 41 per cent at $20$, and 58 per cent at $28.3$: a diffuse excess that ordinary photometry of the cluster excludes at every $\Delta v$ in the bracket. The kinetic channel over the same bracket costs between $2\times10^{-6}$ and $3\times10^{-5}$ of the cluster's light. Gravitational-assist sequencing among cluster stars, which uses other stars as momentum reservoirs and pays only actuator overhead, is the one scheme that survives its own thermal signature; the conclusion is the one earlier drafts reached, and the margin behind it is now three to four orders of magnitude rather than a factor of a few.
...
\node[phase] (p4) at (9.0,0) {\textbf{4. Spin-up}\\ $a_\star\!\to\!0.998$, $10^{4\text{--}5}$ stars\\ $t \sim 3.4$--$5.3\times10^{7}$ yr};
