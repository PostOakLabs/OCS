...
% session; leave \date at "Draft v0.2 (last revised 2026-08-16)".
%
...
\date{August 2026 \\[4pt] \small Draft v0.2 (last revised 2026-08-16) --- methods note for the AXI campaign;\\ \small companion to the mass-tension paper (Paper H); prepared for omegacentauri.me}
...
\section{The question, and the plan it was asked under}
\label{sec:question}
...
The boundary comes from a radius-resolved terminality ruling on Paper H's profile leg. Beyond about $50\as$ the systematics budget runs 10 to 40 times the statistical errors, and those radii carry less than $10^{-3}$ of the dark component's enclosed-mass signal, so the bins that dominate the leg carry no verdict signal. A better error model does not create signal the geometry does not put there.

What the boundary permits is narrow and conditional: the inner bins ($\lesssim 10\as$), where signal-to-systematics is of order unity, are marginal rather than terminal, and a campaign that closed the component-differential residual there might make them usable. That route runs through decision criterion~5 of the plan and needs its own ratified amendment to Paper H's pre-registration. What the boundary forbids is pre-committed and absolute: no re-scoring of Paper H's criteria 2, 3 or 4 from this campaign's output, no restoration of the profile leg to the configuration that Paper H's own misspecification gate dropped, and no relaxation of Paper H's standing embargo list. This note makes no statement of any kind about the cluster's dark component, and it quotes no dark-component posterior.
...
\item[H-B, flattening with azimuthal averaging.] A spherical model fitted to azimuthally averaged data of a flattened system ($\epsilon = 0.17$, \citealt{WhiteShawl1987}, the largest published value; star-count isophotes give 0.11--0.12 with a rounder core, \citealt{Geyer1983,Pancino2003}) produces a component-differential residual by construction. Signature: residual power at $\cos 2\phi$ in position angle, with the split amplitude scaling as the flattening \citep{vanderMarelAnderson2010}.
...
\centering
\small
\caption{The switch behind the X-G-c failure, at the fiducial-bracket MAP cell, Plummer at 5494~pc. Each row is unimodal in the white systematic $s$; the preferred solution switches between rows. Prior shrinkage cannot produce the move, because the outermost knot carries a posterior standard deviation of 0.022 to 0.042~km\,s$^{-1}$.}
\label{tab:switch}
\begin{tabular}{lrrrr}
\toprule
$\tD$ [km\,s$^{-1}$] & $s$ mode & $s$ median & mass at $s < 0.06$ & outer knot \dD{} [km\,s$^{-1}$] \\
\midrule
0.25 & 0.09 & 0.090 & 0.034 & $0.354 \pm 0.042$ \\
0.50 & 0.03 & 0.030 & 0.968 & $1.207 \pm 0.026$ \\
1.00 & 0.03 & 0.030 & 0.969 & $1.209 \pm 0.026$ \\
\bottomrule
\end{tabular}
\end{table}

The mechanism is a degeneracy between \dD{} and the white systematic $s$, and it is measured rather than inferred (Table~\ref{tab:switch}). The outer bins carry a component split that has to be paid for somewhere. At $\tD = 0.5$ or $1.0$ the model pays with a large differential and a small white systematic; at $\tD = 0.25$ the differential is expensive enough that the model pays instead with a white systematic three times larger and inflated errors. The switch is discrete and it lands between 0.25 and 0.5.
...
\subsection{What is not quotable}
\label{sec:notquotable}

Stated explicitly, because the campaign's outcome is a negative one and negative outcomes are the easy ones to overread. No mechanism is identified, and ``cannot identify the mechanism'' is the finding rather than a stand-in for one. No pointwise \dD{} value beyond about $130\as$ is a measurement, because it sits outside the calibrated region. No dark-component number appears anywhere in this note, and criterion~5 stays closed. No compact-versus-extended statement is made in any register, sourced to the profile leg or otherwise. H-B and H-C are not tested and are not disfavoured; the frozen radial table carries neither their data nor their discriminants.
...
\section{What this feeds, and what it does not}
\label{sec:feeds}
