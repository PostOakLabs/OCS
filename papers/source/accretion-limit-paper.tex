...
\textbf{Run-time self-checks.} Two consistency assertions accompany the shipped code and print their result rather than assuming it, the discipline of Paper H's self-audit: the expansion script (\texttt{figs/fF\_expand\_v1.py}) reproduces the RIAF anchors of Table~\ref{tab:anchors} against \texttt{fF\_v3\_results.json} and asserts the match at run time before proceeding; the Haggard luminosity-anchor check above asserts $4\pi D^{2} f_{X,\mathrm{lim}}$ against \citeauthor{Haggard2013}'s stated $1.6\times10^{30}\,\mathrm{erg\,s^{-1}}$ at their own distance. Both assertions halt the build on failure, so a compiled paper carries their pass by construction; neither prints a numeric residual beyond the values already quoted above.
