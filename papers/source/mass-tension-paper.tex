...
\textit{Verification, six routes.} (i) the closed form above; (ii) adaptive quadrature of the original double integral, agreeing to $2\times10^{-13}$; (iii) a Poisson-field Monte Carlo that draws $4\times10^{4}$ realizations of a random stellar background and measures the scale of the resulting line-of-sight jerk distribution without ever evaluating the integral, returning $\xi = 3.489 \pm 0.028$; (iv) symbolic evaluation; (v) numerical differentiation of the antiderivative used in step (ii); (vi) 50-digit arithmetic on the closed form. Route (iii) also confirms the tail index, with a Hill estimator of 1.025 against the 1-stable value of 1 and a Kolmogorov--Smirnov $p = 0.26$ against the analytic law.

\textit{Consequence.} The floor scales linearly in $\xi$, so a jerk-based significance computed with 3.04 is overstated by 12.1\,per~cent in $\dot{a}_{0}$. Figure~\ref{fig:jerk} shows the two floors against the density they set. The correction does not change any conclusion of this paper, whose jerk result is a non-detection either way, and it does change the noise normalization of any published jerk-based central-mass inference that used the smaller constant.
}}
\end{figure}
...
The Maxwellian-based families cannot be matched to this model's kinematics at all. For a King-form lowered Maxwellian and for a hard-truncated isothermal, the truncated second moment caps at $v_{\rm esc}^{2}/2$, while the Bahcall--Wolf consistency relation $\sigma_{1d}^{2} = \Psi/(1+\gamma)$ used throughout demands $3\sigma_{1d}^{2} \simeq 1.1\Psi$ inside the influence radius, where $\gamma + 1 = 2.75$. The matching problem has no solution over the radii the fast stars occupy, and the shipped truncated polytrope sits outside those families for the same reason, its index clipping at the floor of 0.25. A proposed switch to a Maxwellian family therefore has to state first which cusp kinematics it intends to keep. The tracer-cusp bracket disclosed above is the pair of exponent-shift numbers together with that exclusion. Data and script: \texttt{figs/fH\_calc7\_tail\_families.json}, \texttt{figs/fH\_calc7\_tail\_families.py}.
