...
a mechanism rather than a repair. The pessimistic bracket's under-coverage is mean displacement,
not region width. The recovered mass compensates for the visible mass the bracket removes, at an
...
\textbf{The fast stars are the live probe.} They are few, but everything about their leg behaved: stable optimum across every bracket, selection function reproducing the source paper's own contamination arithmetic, agreement with an independent analysis chain, and clean single-leg injection recovery. Of the two named obstacles, calibration on the extended alternative has since been solved for the verdict configuration at the fiducial bracket (the campaign of Section~\ref{sec:record}); what remains is sample size (the all-seven variant fails the count posterior-predictive check in every cell, so the robust-five census is currently the informative one).
