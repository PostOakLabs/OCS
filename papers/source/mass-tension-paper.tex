...
\centering
\small
\caption{The fit of record: calibrated 90\,per~cent regions in $(\mdark, a)$, fiducial M/L bracket (the only calibrated bracket; the pessimistic bracket is uncalibrated in every configuration and its rows are so labelled in the released tables). HPD is the primary estimator; the profile-likelihood (PL) region is the disclosed secondary. All regions abut the $a = 10^{-4}$\,pc grid edge and are one-sided in $a$.}
\label{tab:regions}
\begin{tabular}{lcccc}
\toprule
